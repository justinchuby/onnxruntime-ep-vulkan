...
//! ------------------------------------------------------------------
//! Two reasons, and they are the same two `rust/tests/row_tile_fallback.rs` gives:
//!
//! 1. `rust/tests/layering.rs` forbids `unsafe` anywhere under `src/ops/`, and mutating process
//!    environment on current Rust requires it. So the *parsing* and the *legality* live in
...
//! 2. That pair is not redundant. A refactor that stopped reading the variable, or that read it
//!    and then dropped the result on the floor, would leave every pure unit test green. This is
//!    the test that fails.
//!
...
//! Why no GPU
//! ----------
//! Nothing here builds a pipeline, allocates device memory, or submits anything. The question
//! this instrument answers is *which kernel would be built*, and that question is decided on the
//! host, before any device is touched. Issue #81 asks for the selector to become measurable; a
//! test that needed a device to check the selector would inherit every reason the measurement it
//! unblocks is hard to take in the first place.
...
/// This is the shape every `(16,2)` / `(8,4)` claim in `docs/PERF.md` §25 and §26 is about.
const K: i64 = 3072;
const N: i64 = 3072;

/// Puts the variable back on **every** exit path, including the unwind path.
///
/// A trailing `remove_var` at the bottom of a test does not run when an assertion above it
/// panics, and the value then leaks into whatever runs next. There is exactly one test in this
/// binary that touches the environment, so this guard is a restore discipline and not a second
/// mutex over a global — the single-lock rule in `allocator::ledger` is untouched.
...
/// resulting pipeline the way `vk/session.rs` records it.
///
/// Returns the recorded variant string, or the refusal.
...
/// The assertion for a value the platform can hold but Rust cannot read as `&str`.
///
/// Shared by both arms of the `cfg` fork above so the *claim* is written once and only the way an
/// ill-formed value is constructed differs by platform.
...
/// The same reasoning `rust/tests/row_tile_fallback.rs` states, for the same two globals.
#[test]
fn the_tile_request_reaches_the_specialisation_vector_or_refuses_before_it() {
    ...
    // existed. `(cols=16, rows=2)` at `M = 128` is the approved-head reading, taken off
    // `bench/results/real_model_diagnostics_before_gqa.json` →
    // `runs[].counters.pipeline_variants`, where the tiled arm records
    // `q_gemv_matmul_nbits_f16:32,4,32,0,16,1,2` and `...:128,4,32,0,16,1,2`.
    //
    // The vector is `[wg, bits, block_size, has_zp, cols, packed, rows]`. At `K = 3072` the
    // workgroup is 32 (96 blocks, and 32 divides 96 leaving 3 each), so the whole string is
    // pinned here, not just the two fields this change can move.
    EnvRestore::clear();
    counters::reset();
    assert_eq!(
        translate_and_record(128).expect("the unset control must not refuse"),
        "q_gemv_matmul_nbits_f16:32,4,32,0,16,1,2",
        "unset must select the approved-head tile for this shape, byte for byte"
    );
    assert_eq!(
    ...
    // Decode is the other half of the control: `M = 1` takes `rows = 1`, the specialisation-
    // constant arm holding the verbatim pre-row-tile kernel.
    counters::reset();
    assert_eq!(
        translate_and_record(1).expect("the decode control must not refuse"),
        "q_gemv_matmul_nbits_f16:32,4,32,0,16,1,1",
    ...
    assert_eq!(
        ...
        "q_gemv_matmul_nbits_f16:32,4,32,0,8,1,4",
    ...
    // The request is honoured wherever it is legal, including at `M = 1`. That is deliberate and
    // it is the reason the unset control above is the load-bearing one: an explicit request beats
    // an implicit policy, so decode is only protected by the variable being unset.
    counters::reset();
    assert_eq!(
    ...
    // `(16,4)` is exactly the pair `docs/PERF.md` §25.3 names as "strictly better and refused":
    // `cols*rows = 64` exceeds the accumulator budget `GEMV_MAX_TILE = 32`. Raising that budget
    // is the change this instrument exists to inform; it is emphatically not this change.
    for (value, expect) in [
        ("16,4", "GEMV_MAX_TILE"),
        ("32,1", "GEMV_MAX_COLS"),
        ("4,8", "GEMV_MAX_ROWS"),
    ...
    // A malformed value refuses on syntax, not on legality, and quotes the operator's text back.
    for value in [
        "",
        "8",
        "8, 4",
    ...
    // refused.
    //
    // Both arms, per the P2 portability rule: an environment value is UTF-16 on Windows and bytes
    // on everything else, so an ill-formed one is built differently, but the *assertion* is
    // identical and lives in `assert_non_utf8_refuses` below.
    #[cfg(windows)]
    {
    ...
    // ── THE LATCH CONTROL ────────────────────────────────────────────────────────────────
    // Removing the variable must restore the default, not leave the last request latched. Same
    // property `row_tile_fallback.rs` asserts for the ceiling, and the one a process-global
    // cached selection would break.
    EnvRestore::clear();
    counters::reset();
    assert_eq!(
...
///
/// Reads the Rust rather than restating it, which is the same discipline
/// `bench/test_perf_claims.py::_rule_local_size` applies to the GQA workgroup rule.
///
/// Touches no environment and no counter, so it is safe to run beside the test above.
#[test]
