...
//
// JSON-only, no `abi_version` bump, for the same reason as `pipeline_variants` above.
//
// **Why this is a separate field from `pipeline_variants` rather than a state of it.**
// `ONNXRUNTIME_EP_VULKAN_GEMV_TILE` is honoured by *building the pipeline it names*, so a request
// that succeeds is already witnessed — index 4 and index 6 of the recorded specialisation vector
// are the resolved `(cols, rows)`, recorded from the effective pair handed to `get_or_create` and
// not from what the environment asked for. That is the readback witness and this field must not
// duplicate it.
//
// A request that is **refused** has the opposite problem: it produces no pipeline at all, so
// `pipeline_variants` is silent about it in a way that is indistinguishable from the variable
// never having been set. R12 — a silence is not a zero. This field carries that one fact, and
// only that fact: the reason text of the first refusal, or `"NONE"`.
//
// `"NONE"` is honest for both "no request was made" and "the request was honoured", because in the
// second case the honoured pair is visible in `pipeline_variants` and reading both together is
// unambiguous. What is *not* representable, deliberately, is a refusal that left no trace.
// ---------------------------------------------------------------------------
...
/// The refusal reason, or `"NONE"`. See the module note above for why `"NONE"` is not ambiguous.
...
pub fn reset() {
    ...
    // Same reason as the two above: a refusal left latched across a reset is one test reading
    // another test's misconfiguration.
    if let Ok(mut slot) = GEMV_TILE_REQUEST_REFUSED.lock() {
        *slot = None;
    }
