...
//!   `ops::partition::is_anchor` — its weight matrix is a resident initializer at a
//!   schema-designated site — so a lone `Gemm` island is exempt from the minimum-node gate; the
//!   row was already written into the partitioner's cost model before any kernel existed.
...
mod tests {
    ...
    /// `Gemm` is a partition anchor **when its weight matrix is resident**, which is why a lone
    /// one at a model's tail is claimable at all. Since issue #73 that is a property of the node
    /// and not of the op's name: a `Gemm` of two activations takes no exemption.
    #[test]
    fn gemm_is_a_partition_anchor_when_its_weight_is_resident() {
        use crate::ops::partition::is_anchor;
        // MobileNetV2-12's classifier head: A activation, B resident, C resident bias.
        assert!(is_anchor("Gemm", &[false, true, true]));
        assert!(is_anchor("MatMul", &[false, true]));
        // The bias alone is not a weight site, and neither op anchors on activations.
