...
// ──────────────────────────────────────────────────────────────────────────────────────────────
// The exact-tile request — an instrument, and deliberately not an optimisation
// ──────────────────────────────────────────────────────────────────────────────────────────────

/// Names an **exact** `(cols, rows)` tile for `MatMulNBits`, when an operator wants one.
///
/// # Why a second knob beside `ONNXRUNTIME_EP_VULKAN_GEMV_MAX_ROWS`
///
/// The existing knob is a **ceiling**: it can only remove candidates, so every pair it can reach
/// is a pair [`gemv_tile`] would have chosen for *some* ceiling. One pair it cannot reach is the
/// pair worth measuring.
///
/// When `cols` divides `N`, [`gemv_named_bytes`]'s weight term depends only on `rows` and its
/// activation term only on `cols`. `(16, 2)` and `(8, 4)` therefore name **exactly equal** bytes on
/// the Phi-3.5 projection shapes, and [`gemv_tile_with`]'s strict-`<` improvement rule keeps the
/// incumbent on a tie. A ceiling does not break ties; it only lowers them. That is an arithmetic
/// property of the two functions in this module, and it is asserted directly by
/// `the_equal_traffic_arm_is_legal_and_the_incumbent_it_ties_with_is_too` rather than inferred
/// from any run.
///
/// `(8, 4)` is the one arm that isolates the register-pressure and occupancy cost of `rows = 4` at
/// *identical named traffic*, which is the risk gating any future widening of [`GEMV_MAX_TILE`].
/// Until this variable existed, no surface in the tree could select it, so that risk is
/// **UNMEASURED** — which is not the same fact as measured-and-negative.
///
/// # What it is not
///
/// It takes no reading and changes no default. Unset, [`gemv_tile_selection`] *is* a call to
/// [`gemv_tile`]: identical by construction, not by resemblance. Set and legal, it selects the
/// named pair exactly. Set and illegal or malformed, it **refuses before dispatch** — never
/// clamps, never ignores, never falls back. A knob that quietly downgrades an impossible request
/// is how a reading comes to describe a configuration nobody ran, which is the defect class
/// `ONNXRUNTIME_EP_VULKAN_DEVICE` produced once already when `DEVICE=0` named what was asked for
/// while the work happened on device 1.
const GEMV_TILE_VAR: &str = "ONNXRUNTIME_EP_VULKAN_GEMV_TILE";

/// Why a tile request was refused. Both variants carry the request, so a diagnostic can quote the
/// operator's own text back instead of paraphrasing it.
...
pub enum TileRequestError {
    ...
    /// The value parses, and names a tile some rule this build already enforces forbids.
    Illegal {
        ...
        match self {
            ...
            Self::Illegal { cols, rows, detail } => write!(
                ...
                "{GEMV_TILE_VAR} requested the tile cols={cols} rows={rows}, which this build \
                 cannot dispatch: {detail}. Refused rather than clamped — a tile the kernel would \
                 not have run is not a tile any reading may be attributed to"
            ),
        }
    }
}
...
///
/// `None` means unset, and unset is not an error — it is the one input for which the caller must
/// behave exactly as it behaved before this function existed.
///
/// The grammar is narrow on purpose. Every rejection below is a spelling a lenient parser would
/// have accepted, and leniency is what makes an instrument untrustworthy: if `" 8 , 4 "`, `"8,4,"`
/// and `"8,4junk"` all quietly mean `(8, 4)`, the value an artifact records no longer identifies
/// what the operator typed, and a typo becomes indistinguishable from an intent.
///
/// * Exactly one comma, with a non-empty field on each side.
/// * Each field is ASCII digits only — no sign, no whitespace anywhere, no underscore, no radix
///   prefix, no exponent, no non-ASCII digit.
/// * At most 10 digits, and the value must fit in `u32`. A 20-digit number is a typo, not a tile,
///   and wrapping it into a small plausible tile would be the worst available answer.
/// * Both values are at least 1; `0` names an empty dispatch.
/// * The empty string is a syntax error rather than "unset". On Windows `set VAR=` removes the
///   variable, so an empty value generally reaches here only from a caller that built it — which
///   is exactly when it should be refused.
pub fn parse_tile_request(raw: Option<&str>) -> Result<Option<(u32, u32)>, TileRequestError> {
    ...
    let (Some(cols_s), Some(rows_s)) = (fields.next(), fields.next()) else {
        return Err(syntax(
            "there is no comma, so this names one number and not a pair",
        ));
    };
    if fields.next().is_some() {
        return Err(syntax(
            "there is more than one comma, so this names more than a pair",
        ));
    }
    let field = |s: &str, is_cols: bool| -> Result<u32, TileRequestError> {
        if s.is_empty() {
            return Err(syntax(if is_cols {
                "the columns field is empty"
            } else {
                "the rows field is empty"
            }));
        }
        if !s.bytes().all(|b| b.is_ascii_digit()) {
            return Err(syntax(if is_cols {
                "the columns field holds a character that is not a base-ten digit"
            } else {
                "the rows field holds a character that is not a base-ten digit"
            }));
        }
        if s.len() > 10 {
            return Err(syntax(if is_cols {
                "the columns field has more digits than any u32 has"
            } else {
                "the rows field has more digits than any u32 has"
            }));
        }
        let v: u32 = s.parse().map_err(|_| {
            syntax(if is_cols {
                "the columns field does not fit in a u32"
            } else {
                "the rows field does not fit in a u32"
            })
        })?;
        if v == 0 {
            return Err(syntax(if is_cols {
                "zero columns names an empty dispatch"
            } else {
                "zero rows names an empty dispatch"
            }));
        }
        Ok(v)
    };
    Ok(Some((field(cols_s, true)?, field(rows_s, false)?)))
}

/// Is `(cols, rows)` a tile this build could already have dispatched? **Pure.**
///
/// Every clause is a rule that already exists, restated against a *requested* pair rather than a
/// searched one. None of them is new, and that is the point: the override may move a measurement
/// onto a different pipeline, and may never move one onto a pipeline the shipping selector was
/// forbidden to build.
///
/// * `rows <= GEMV_MAX_ROWS` — [`gemv_tile_with`]'s search bound, and `q_gemv.comp`'s `QB_MAX_ROWS`
///   activation window.
/// * `cols <= GEMV_MAX_COLS` — [`gemv_cols`]'s seed, and the shader's `QB_MAX_COLS`-wide arrays.
/// * `cols * rows <= GEMV_MAX_TILE` — the accumulator register budget the shader sizes `acc`/`bacc`
///   by, which `matmul_nbits_gemv`'s fail-closed guard and the shader's own folded spec-constant
///   guard both already enforce.
/// * `wg * cols <= GEMV_RED_WORDS` — the shared reduction array. `q_gemv.comp` declares
///   `shared float red[2048]`, which is **8 KiB**: half of the 16 KiB `maxComputeSharedMemorySize`
///   floor, deliberately, so a device that only meets the floor still has room for a second
///   resident workgroup. The 8 KiB is what the module spends; the 16 KiB is what the specification
///   promises. They are different numbers and this bound is about the first.
/// * `cols` divides `N` — no tail column tile. The shader is correct with one (out-of-range
///   columns are redirected and re-checked at the store) but the paired non-atomic store is only
///   taken when the tile owns both fp16 lanes, so a tail tile silently changes which store path a
///   reading exercises.
/// * `cols == 1 || N / cols >= GEMV_MIN_WORKGROUPS` — a narrow output must not trade all of its
///   parallelism for reuse.
/// * `N / cols <= GEMV_MAX_GROUPS_Y` — the dispatch's **x** extent, which nothing else bounds and
///   which has no grid-stride loop. The shipping selector cannot reach a bad value (`gemv_cols`
///   seeds at `min(16, …)` and only halves), but a request can: `cols = 1` also bypasses the
///   parallelism clause, so `1,1` against a large-vocabulary `lm_head` asks for `groupCountX == N`.
///   Above the guaranteed `maxComputeWorkGroupCount[0]` floor that dispatch is not slow, it is
///   invalid (`VUID-vkCmdDispatch-groupCountX-00417`).
///
/// `m` and `blocks_per_col` are checked for a different reason than the rest: they decide whether
/// the request is *meaningful* rather than whether it is safe. A `rows` above `m` is permitted
/// deliberately — it is what the last row tile of any prefill already does, and refusing it would
/// make the tail geometry unmeasurable on purpose.
...
pub fn tile_request_legality(
    ...
    if red > GEMV_RED_WORDS {
        return Err(illegal(format!(
            "wg*cols={red} exceeds GEMV_RED_WORDS={GEMV_RED_WORDS} at wg={wg}; that is the shared \
             reduction array `shared float red[{GEMV_RED_WORDS}]`, 8 KiB of the 16 KiB floor the \
             specification guarantees, and no measurement may spend the half the module leaves \
             free for a second resident workgroup"
        )));
    }
...
///
/// The **only** consultant of the environment in this area. [`gemv_tile`] stays pure and stays the
/// selector; this wraps it so that "unset" is not merely equivalent to the previous behaviour but
/// is literally a call to it — which is also what keeps `bench/results/probe_weight_reread.py`'s
/// mirror of the selector a mirror of the thing that actually runs.
#[allow(clippy::too_many_arguments)]
pub fn gemv_tile_selection(
    ...
    // `var_os`, not `var`. `std::env::var` collapses `NotPresent` and `NotUnicode` into one `Err`,
    // so `.ok()` would read a mangled value — a driver script that built the string badly — as
    // "unset", and the run would take the searched tile while the operator believed it took
    // theirs. That is the same silent substitution this variable refuses to perform for an illegal
...
fn matmul_nbits_gemv(
    ...
    // The tile: the byte model's choice, or the exact pair an operator named.
    // `gemv_tile_selection` is the only consultant of the environment here, and with the variable
    // unset it *is* a call to `gemv_tile` — so the unset path is identical by construction rather
    // than by resemblance.
    //
    ...
    // specific unmeasured configuration measurable, and an instrument that silently substituted a
    // different configuration for the requested one would attribute the reading to the wrong
    // kernel — the whole failure `pipeline_variants` was added to close.
...
mod tests {
    ...
    // ──────────────────────────────────────────────────────────────────────────────────────
    // The exact-tile request — grammar
    // ──────────────────────────────────────────────────────────────────────────────────────

    /// Unset is not a value. The clause the whole instrument rests on: with the variable absent
    /// the caller must take the *same code path* it took before this function existed, not a path
    ...
    /// The pairs an operator is actually going to type, including the one this exists for.
    #[test]
    fn a_well_formed_tile_request_parses_to_exactly_the_pair_it_names() {
        ...
        assert_eq!(
            ...
            Ok(Some((8, 4))),
            "a leading zero is still base ten, and the recorded witness is the resolved pair \
             rather than the spelling, so this needs no second rule"
        );
    }

    /// **The refusal table.** Every row is a spelling a lenient parser would have accepted, and
    /// accepting any of them means an artifact's recorded value stops identifying what the
    /// operator asked for.
    ///
    /// Deliberately not a loop over one nameless assertion: each row names the rule it exercises,
    /// so a regression says *which* rule went missing rather than only that something did.
    #[test]
    ...
    /// A number too large for a `u32` must fail as a number rather than wrap into a plausible
    /// tile. `4294967296` is `u32::MAX + 1`; wrapping it would make `(0, 4)`, which the zero rule
    /// would then reject for the wrong reason.
    #[test]
    fn an_overflowing_tile_request_refuses_instead_of_wrapping() {
        ...
        assert_eq!(
            ...
            Ok(Some((u32::MAX, 1))),
            "the largest u32 is still a parse; refusing it is the legality check's job"
        );
    }

    /// The refusal text has to name the variable and quote the operator's own value back, or a
    /// reader has to guess which switch produced it.
    #[test]
    fn a_refusal_names_the_variable_and_quotes_the_request() {
        let text = parse_tile_request(Some("8, 4")).unwrap_err().to_string();
        assert!(
    ...
    // ──────────────────────────────────────────────────────────────────────────────────────
    // The exact-tile request — legality
    // ──────────────────────────────────────────────────────────────────────────────────────

    /// One Phi-3.5 projection shape, `K = N = 3072`, which is the shape the `(16,2)`/`(8,4)`
    /// argument is about.
    ...
    /// **The arm this instrument exists to make reachable**, and the tie that makes it
    /// unreachable without one.
    ///
    /// Both halves are arithmetic over this module's own functions — no run is cited and none is
    /// needed. If `gemv_named_bytes` ever stopped tying the two pairs, this instrument would be
    /// moot, and the failure would land in the file that documents it.
    #[test]
    fn the_equal_traffic_arm_is_legal_and_the_incumbent_it_ties_with_is_too() {
        ...
        // And no ceiling reaches it, which is the reason a second knob is not redundant. Stated
        // as an exhaustive check over the ceiling's whole legal range rather than as a claim
        // about any number of runs.
        for ceiling in 1..=GEMV_MAX_ROWS {
            assert_ne!(
                gemv_tile_with(128, 3072, 3072, 4, 2, wg, ceiling),
                (8, 4),
                "no value of the ceiling may reach (8,4); ceiling={ceiling}"
            );
        }
    }
    ...
    fn a_tile_request_may_not_reach_past_any_bound_the_selector_respects() {
        // The accumulator register budget. `(16,4)` is exactly the pair the selector refuses as
        // "strictly better and illegal", and it must stay refused here for the same reason.
        assert!(
        ...
        // An empty dispatch, both ways it can be empty.
        assert!(matches!(
    ...
    /// Overflow in the *bounds* check, stated honestly.
    ///
    /// The caps run first, so `u32::MAX` columns is refused by `GEMV_MAX_COLS` long before
    /// `cols.checked_mul(rows)` is reached. Those `checked_mul`s are defence in depth against a
    /// future reordering, not a live path, and a test claiming to exercise them would be claiming
    /// something false. What is checked is the property an operator cares about: a gigantic
    /// request is refused by a *named* rule rather than wrapping into a small legal-looking
    /// product.
    #[test]
    fn a_huge_tile_request_is_refused_by_a_named_cap_before_anything_can_wrap() {
        for (cols, rows, expect) in [
            ...
            let err = tile_request_legality(cols, rows, 128, 3072, 3072, 32, 96)
                .expect_err(&format!("({cols},{rows}) must refuse"));
            assert!(
    ...
    /// The shared reduction array, refused — the one bound with a specification number behind it,
    /// and the one that had only a boundary `Ok` and no case showing it fire.
    ///
    ...
    fn a_request_that_would_overrun_the_shared_reduction_array_is_refused() {
        ...
        assert!(
            text.contains("GEMV_RED_WORDS"),
            "the refusal must name the shared array: {text}"
        );
        assert!(
            text.contains("8 KiB") && text.contains("16 KiB"),
            "the refusal must distinguish what the module spends (2048 floats = 8 KiB) from the \
             floor the specification promises (16 KiB): {text}"
        );
        // The boundary immediately below is still allowed, or the clause is off by one.
        assert_eq!(
            tile_request_legality(8, 4, 128, 3072, 16384, wg, 512),
            Ok(())
        );
        // And the exactly-at-the-bound case at the workgroup size K=8192 takes.
    ...
    /// `cols = 1` also bypasses the parallelism clause, so it is the one combination that can ask
    /// for an *invalid* `vkCmdDispatch` rather than a slow one.
    #[test]
    fn a_request_whose_x_grid_exceeds_the_guaranteed_floor_is_refused() {
        let wg = gemv_workgroup(96);
        let n = 128_256u64; // a large-vocabulary lm_head; nothing in the claim predicate bounds N
        ...
        assert!(
            gemv_cols(n, wg) > 1,
            "the searched selector must not be able to produce cols=1 here, or this is not a \
             request-only hazard"
        );
        assert_eq!(tile_request_legality(16, 2, 128, n, 3072, wg, 96), Ok(()));
    }

    /// A legality refusal has to say which rule and with which numbers, or an operator is left
    /// bisecting a table of bounds by hand.
    #[test]
    fn a_legality_refusal_names_the_rule_and_the_numbers_that_broke_it() {
        let text = phi35_legality(16, 4).unwrap_err().to_string();
    ...
    /// `rows > m` is legal on purpose: it is what the last row tile of any prefill already does,
    /// and refusing it would make the tail geometry unmeasurable.
    #[test]
