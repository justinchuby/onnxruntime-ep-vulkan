...
#
# `instrument_census.json` carried the sentence "85 bench rows read `unfalsified`" beside
# a MACHINE-GENERATED `bench_unfalsified` array holding 162 of them, and a hand-classified
# entry that stated its own row count three times and was wrong all three. Every array
# matched the tree, so all four drift comparisons passed: the census cannot see a summary
# that disagrees with the collection it summarises, because the summary was never an
...
def test_a_hand_edited_summary_count_is_drift() -> None:
    ...
    audit = _load_audit()
    base = _baseline(audit)
    mutated = dict(base["counts"], bench_unfalsified=85)
...
# derives twenty-two of them: six array lengths and sixteen per-module counts. That is a
# large enough bag that a wrong sentence can nearly always find a witness in it. The
# specimen the review found is `18 rows across these four modules`: 18 is real — it is
# `bench/cuda_competition.py` alone — and the four modules the entry names sum to 43.
#
# So a claim that names a scope is bound to that scope. A module-set claim must equal the
# sum over the modules ITS OWN ENTRY declares, and its cardinality word must equal how
...
#
# Both polarities on every arm, and the numbers below are read from the census rather
# than written out, so these tests cannot themselves become the stale figure.
# ---------------------------------------------------------------------------
...
def test_the_cardinality_word_is_bound_too() -> None:
    """"these three modules" beside four module names is a false sentence with a true sum."""
    audit = _load_audit()
    base = _baseline(audit)
    counts = audit.baseline_counts(base)
    _, _, _, total = _four_module_entry(audit, base)
    ...
    offenders = audit.prose_row_claims(
        _with_claim(audit, base, f"{total} rows across these three modules"), counts)

    assert offenders and "module(s), not 3" in offenders[-1]
