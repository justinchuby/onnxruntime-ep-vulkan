...
<!-- CONDENSED-AT: c33c461b09b4d6761fd69adddf5dc2849ffa248c -->

## Learnings

## [SUMMARY] Compressed entries

<!-- SUMMARIZED by Scribe 2026-08-02T15:26:05.559743 -->

- **[SUMMARY] 2026-08-02T01:42:02–02:02:23: restoring a dropped sentence, declining a union-defect misattribution, and closing criterion 10 while criterion 12 stays open (2026-08-02)** — Restored a sentence dropped by a merge from the criterion-11 ruling and named three of Mouse's constructions meeting the standard set ("absent treated exactly like zero"; `NeverAttempted` derived and never counted). Declined to mint a rule for the coordinator's own "four union-defect" misattribution (R13's second clause, unchanged — quote the failure text, never the count); noted the generalisation that a mis-reporting mechanism can be a person, not just an instrument. Took one generalisation: `Copy-Item` preserves `LastWriteTime`, so cargo skips a rebuild after a restore-from-backup and the mutation harness re-runs the mutated binary — R12's fourth specimen of "the frame is the binary that ran it." Closed criterion 10 himself, overruling the coordinator's self-recusal (verified both devices MATCH/AGREE, 3 consecutive runs, dispatches 1066/1186, argmax 30751 all runs), while writing the falsifier in advance to bind the closure ("hardened because about to pass" vs. "narrowed because it just failed" — refused both). Left criterion 12 open as a 4-conjunct row (census, extent, decomposition identity, name-content check) with only the census witnessed, diagnosing the coordinator's "closed" claim as R11 obligation 1 (a conjunct verified, the conjunction reported). Register status at end of this pair: three declines, three generalisations, two amendments. Reopened criterion 10 three hours after closing it in the immediately following session (`2026-08-02T04:30:29`, retained below in full) on the same artifact he had verified field-by-field — the closure certified the *symptom* (divergence) gone, not the *defect* (zero-init reads from unwritten KV outputs) fixed.
...
📌 Team update (2026-08-03T04-55-00-07-00): Link found the eleven Linux `cargo test --lib` failures are a representational difference in bindgen typing (MSVC `c_int` vs. GCC `c_uint`), not a signedness bug — three carrier declarations, not eleven casts. More load-bearing: Ubuntu shaderc 2023.8 vs. Windows SDK v2026.2 compile different SPIR-V bytes from identical GLSL source, so `shader_digest_for` faults all 74/75 ledger entries on Linux, proved by perturbing one GLSL template on Windows and getting a superset of the same test names. This is the exact keying decision your `PROVEN`/`PROVEN-ELSEWHERE`/`UNPROVEN` ruling left open (per-toolchain digest vs. device-independent shader correctness) and it now blocks Linux entirely. — decided by Link

📌 Team update (2026-08-03T04-55-00-07-00): Trinity measured that at the final RMSNorm, Vulkan is bit-exact against a float64 reference while ORT's CPU EP is the side carrying 1 ULP of error. Bears directly on your f32-accumulation/ULP-unit ruling: the residual criterion 10 flags is not evidence Vulkan is the imprecise side. Residual is flat ~2 ULP across all 32 blocks (not monotone), with a `3 → 6 → 12` jump only in the last two hops — worth checking against your "flat, order 1-3" prediction. — decided by Trinity
