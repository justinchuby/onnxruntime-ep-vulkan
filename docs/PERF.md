...
| `outputs_dir`, `scratch_dir`, `profile_path`, `trace_path` | *evidence* — a rooted path per §4.0.1 | a reader; and `--reanalyse`, via `public_paths.resolve_public_path` | yes, rooted |
...
repaired. `resolve_public_path` is the inverse of `public_path` and is what makes a rooted path
*evidence a second machine can act on*: `--reanalyse` re-derives equivalence from a committed
record whose tensors sit under the reader's own checkout.
