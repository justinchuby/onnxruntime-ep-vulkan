...
| `GroupQueryAttention` | T3 | Mouse | **DELIVERED 2026-07-30.** Decode path (`seq_len=1`), packed QKV, `do_rotary=1`, neox RoPE, online-softmax GQA kernel (`gqa_f16.comp`), in-place KV-cache via aliased output. Phi-3.5: Intel MATCH 618 ms, NVIDIA MATCH 230 ms, 353 claimed, 1 island. Prefill (`seq_len>1`) needs inter-invocation sync and is not yet claimed. Declines `local_window_size ≠ -1`, `softcap ≠ 0`, `smooth_softmax > 0`. **Second module 2026-08-09 (#90):** the same claim now dispatches one of **two** shaders — `gqa_decode_f16.comp`, a KV-parallel decode kernel (one workgroup per `(batch, query head)`, 2–16 lanes striding the cache, shared-memory halving-tree merge, Vulkan 1.1 core only), or `gqa_f16.comp` unchanged for prefill and for every form the selector refuses (`seq_len ≠ 1`, `head_dim ∉ 1..=128`, unrecoverable extent, or a cache short enough that the ladder lands on one lane). Claim predicate, attribute declines and the `gqa_f16` SPIR-V are all unchanged; the proof key gains an `@kvpar` suffix naming both modules. **No speedup is claimed** — see `docs/PERF.md` §28. |
