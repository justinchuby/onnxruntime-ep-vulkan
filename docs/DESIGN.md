...
>
> Last re-derived from artifacts: **2026-08-02, at `main` = `6ef62bb`.**

### 0.1 What it does

**It loads into a stock ORT and claims a real model.** On Phi-3.5-mini-instruct (ONNX, int4
`MatMulNBits`, 363 nodes), the EP claims **355 nodes** and ORT fuses them into **one island**:
`bench/results/phi35_claim_reading_summary.json` — `claimed_nodes: 355`, `islands_offered: 1`,
`viable_islands_retained: 1`, `ledger_hits: 355`, `unproven_forms_claimed: 0`. The remaining nodes
...
**Its op table is 91 rows, of which 71 carry a kernel.** Counted from the `ops!` rows in
`rust/src/ops/{elementwise,norm,attention,quant,indexing,moe,ssm}.rs`: 71 rows are `Live`/`Ready`
(67 `ai.onnx`, 4 `com.microsoft`) and **20 are `Staged`** — described, claim-tested, and declined at
runtime with a named blocker. `OpStatus::Live` is a deprecated alias of `OpStatus::Ready`
(`rust/src/registry.rs::OpStatus`); neither grants a claim. **Claimability is a ledger fact, not a
table field.**

**Nothing is claimed without a proof under its own key.** `evidence/proof_ledger.jsonl` holds **97
entries** covering **69 distinct (op, domain) pairs**, each recording the device, the ORT build, the
tolerance, the artifact, the nodes claimed and dispatches executed on the proof run, the shader stems
dispatched, and a digest of exactly those stems' SPIR-V. A form with no entry declines with
`[unproven]`; a build whose kernels have moved declines too (§8.9.17). The gate has been shown to
move with its input rather than with the enumeration that feeds it:
`bench/results/census/criterion11c-ledger-arms-dev0.json` reads `ALL-PROVEN`/`ALL-DECLINED` across
four arms differing in exactly one proof-key component each, and
...
**Its Linux lane compiles and runs, and cannot produce an op-correctness number.** Until 2026-08-03
`cargo test --lib` did not compile on Ubuntu — 11 bindgen-typing errors, repaired at `8e47d3a` as a
**representational** difference in three carrier declarations (MSVC emits `OrtLoggingLevel` as `c_int`,
GCC as `c_uint`; a C enum with no negative enumerator), not as a signedness bug and not with `as i32`.
Four of the seven previously-gated steps now pass. **Three still fail, all from one cause, and the
cause is not the platform:** Ubuntu ships shaderc 2023.8 and the Windows SDK v2026.2, so the SPIR-V
differs, so `registry.rs::shader_digest_for` faults every ledger entry and Linux declines every form.
This was shown to be the ledger rather than Linux by perturbing one GLSL template **on Windows** and
reproducing the same failures as a superset. §8.9.19 rules on what a Linux run may claim; the change is
owed and not landed. **Criterion 1 is not met.**

**The op suite is red.** `pytest tests/ops` does not pass; six failures remain at `6ef62bb`. M0
criterion 2 says *green*, and green is not what the suite reports.
...
**Criteria 2 and 10 are reopened and 11 and 12 are not closed.** See the M0 table in §10. Criterion 10
is `DIVERGENT` on both devices and must stay that way until the three outlying outputs are explained or
fixed; **it must not be closed by moving `atol`.**
...
**A proof is not portable, and the ledger does not yet say so.** All 97 entries record
`"device": "device0"` — and `device0` is a *selector ordinal*, not a device identity
(`rust/tools/gen_proof_ledger.py`: `device = args.device_name or f"device{args.device}"`). No predicate
reads the field. Running on a second GPU claims forms proved on the first, silently. §8.9.17 rules on
this; the change is owed and not landed.

**The ledger is keyed per toolchain — and that is a mechanical accident, not a design.**
`registry.rs::shader_digest_for` hashes compiled SPIR-V bytes, so a different `glslc` invalidates every
entry at once even when no GLSL changed; and because `parse_ledger` `continue`s past a mismatched entry,
the entry never enters `Ledger::entries`, so **a frame mismatch is indistinguishable from a form nobody
ever proved**. §8.9.17 rules that the digest's breadth is protective and must be *declared* rather than
narrowed; §8.9.19 rules that the toolchain belongs to the entry's **frame** and never to its key, that
the same is true of the device, and that the subject therefore needs **two** digests — one over the
SPIR-V and one over the source closure — because no single hash can be sensitive to the kernel and blind
to the compiler.

**No performance claim is made here.** The development machine is permanently contended by several
agents; every duration in this repository is either withheld or carries a device-state record. §0
quotes counts and bytes only.
