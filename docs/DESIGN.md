...
**It does not build its own test suite on Linux.** `cargo build --release` is clean on Ubuntu, but
`cargo test --lib` does not compile — 11 `i32`/`u32` bindgen-typing errors in `rust/src/ep.rs`. The CI
step that dies on them is `.github/workflows/ci.yml` → *"Clippy (all warnings as errors)"*, because it
runs `--all-targets`; the name is a lint's and the defect is a portability defect. A red step skips the
rest of its job, so **seven Linux steps including op-correctness have never executed** — status
`GATED_NEVER_RUN`, not "never observed to fail". **Criterion 1 is not met.**
...
**The ledger is keyed per toolchain.** `registry.rs::shader_digest_for` hashes compiled SPIR-V bytes,
so a different `glslc` invalidates every entry at once even when no GLSL changed. §8.9.17 rules that
this breadth is protective and must be *declared* rather than narrowed — and that a run faulting 97
entries on a compiler upgrade must not look like 97 kernels changed.
