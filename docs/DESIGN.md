...
**Its op table is 92 rows, of which 74 carry a kernel.** Read from
`epctl --dump-capabilities --json` (`rust/src/bin/epctl.rs`), not from counting `ops!` lines: 46
rows report `live`, 28 report `ready` and **18 report `staged`** — described, claim-tested, and
declined at runtime with a named blocker carried in the dump's own `staged_reason` field. The same
figures are `docs/OP_COVERAGE.md` §13.9.6's "after" row. **This count has been misstated in this
document and in status reports more than once; the dump is the only reading of it that is not a
hand tally.** `OpStatus::Live` is a deprecated alias of `OpStatus::Ready`
