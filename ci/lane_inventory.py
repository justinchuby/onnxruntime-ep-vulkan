...
CHECKS: tuple[Check, ...] = (
    ...
    Check(
        ...
        misses=(
            ...
            "movement was OBSERVED to invalidate a green landing check, when PR #67 moved "
            "`main` to 8e6417e on its own schedule and two pull requests that had just gone "
            "green on all four contexts both read `mergeable_state: behind`; the merge was "
            "then refused with HTTP 405 rather than merely labelled. Direct pushes are refused "
            "too (GH013). What is NOT done is the `main_is_green` open red, which stays open "
            "on purpose: it censuses the LAST TEN pushes and three of those are still the "
            ...
            "IT CAN NOW DEADLOCK THE REPOSITORY, and that is the price of the rule rather than "
            "an objection to it. A required context is a STRING in settings and the job that "
            "produces it is a string in `.github/workflows/ci.yml`; rename the job and the "
            "context never arrives, so every pull request waits forever — including the one "
            "that would rename it back. PR #67 edited `ci.yml` in the hour this rule went live "
            "and happened not to touch a `name:`. Happening not to is not a control, so "
            "`ci/test_lane_checks.py::test_every_required_context_is_a_job_this_workflow_still_"
            "produces` holds the two strings against each other in the lane, on the branch, "
            "before the rename can reach the policy.",
