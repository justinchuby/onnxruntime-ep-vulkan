...
#: The fields that state a Vulkan-vs-CUDA speed result, and the only fields in this
#: report that compare two arms against each other.
#:
#: THEY ARE NOT DERIVED FROM MEDIANS. They were, and that was the defect: two medians
#: divided by each other, published whatever the arms had computed and whatever their
#: provider attribution said. A ratio between an arm that ran on the CPU because the
#: provider never claimed the graph, and an arm whose logits are a thousand times the
#: reference's, is arithmetic — it is not a performance result, and nothing in the record
#: said so.
#:
#: :func:`speed_claim` decides; :func:`seal_speed_claim` is what makes the decision
#: structural. A refusal REMOVES these keys rather than relabelling a verdict beside
#: them, because a reader quotes the number and not the label — that is the same lesson
#: :func:`seal_verdict` exists for, applied to the cross-arm claim.
SPEED_FIELDS = ("gap_ms", "speedup_vulkan_over_cuda")

SPEED_COMPARED = "SPEED_COMPARED"
SPEED_REFUSED = "SPEED_REFUSED"

#: The arm this profile compares its Vulkan median against, and the arm both are checked
#: for output equivalence against. Identical to `cuda_competition`'s: one contract, so a
#: claim this module publishes and a claim the suite publishes cannot mean different
#: things by the same word.
SPEED_SUBJECT_ARMS = (cc.ARM_VULKAN, cc.ARM_CUDA)
SPEED_REFERENCE_ARM = cc.ARM_CPU_HOST
...
def speed_claim(untraced: "dict | None", cuda: "dict | None",
                reference: "dict | None" = None) -> dict:
    """May this profile publish a Vulkan-vs-CUDA speed field, and on what evidence?

    Total: it returns a verdict for every input, never raises, and always names the arms
    it was asked about. ``SPEED_COMPARED`` requires ALL of:

    1. Both contributing arms exist and are ``ADMISSIBLE`` — the same admissibility token
       `cuda_competition` gates its own ratio on, so an arm whose provider never claimed
       the graph cannot contribute a median here either.
    2. A reference arm was dispatched. Without one there is nothing to check equivalence
       against, and "we did not check" must not be spelled the same way as "they agreed".
    3. :func:`cuda_competition.cross_arm_equivalence` — the SAME instrument, over the
       records actually dispatched — returns ``MATCH`` for both contributing arms.

    Anything else is ``SPEED_REFUSED`` with the reasons listed. The equivalence result is
    carried on the claim whatever the verdict, so a reader can see which arm failed and
    why rather than only that something did.
    """
    refusals: "list[str]" = []
    dispatched: "list[dict]" = []
    for arm, rec in ((cc.ARM_VULKAN, untraced), (cc.ARM_CUDA, cuda)):
        if not rec:
            refusals.append(
                f"the {arm} arm was not dispatched, so the ratio has no {arm} side; a "
                f"missing arm is not a losing arm and no speed field is quotable")
            continue
        rec = dict(rec)
        rec.setdefault("arm", arm)
        dispatched.append(rec)
        if rec.get("verdict") not in cc.GREEN_VERDICTS:
            refusals.append(
                f"the {arm} arm's admissibility verdict is {rec.get('verdict')!r}, not "
                f"{cc.ADMISSIBLE}; its median does not describe work the provider it "
                f"names actually did")

    claim: dict = {
        "verdict": SPEED_REFUSED,
        "fields": list(SPEED_FIELDS),
        "contributing_arms": list(SPEED_SUBJECT_ARMS),
        "reference_arm": SPEED_REFERENCE_ARM,
        "refusals": refusals,
    }

    if reference:
        reference = dict(reference)
        reference.setdefault("arm", SPEED_REFERENCE_ARM)
        eq = cc.cross_arm_equivalence(dispatched + [reference],
                                      reference_arm=SPEED_REFERENCE_ARM)
        claim["equivalence"] = eq
        for arm in SPEED_SUBJECT_ARMS:
            entry = (eq.get("arms") or {}).get(arm)
            if not entry or not entry.get("verdict"):
                refusals.append(
                    f"the {arm} arm carries no output-equivalence verdict; an unchecked "
                    f"arm is not an equivalent arm, and an empty verdict is a refusal "
                    f"rather than a pass")
            elif entry["verdict"] != "MATCH":
                refusals.append(
                    f"the {arm} arm's outputs are {entry['verdict']} against the "
                    f"{SPEED_REFERENCE_ARM} reference ({entry.get('detail') or 'no detail'}); "
                    f"a speed ratio between arms computing different answers is not a "
                    f"performance result")
    else:
        refusals.append(
            f"no {SPEED_REFERENCE_ARM} reference arm was dispatched, so neither "
            f"contributing arm's outputs were checked against anything. This is the state "
            f"that used to publish a ratio, and it is a refusal")

    if not refusals:
        claim["verdict"] = SPEED_COMPARED
    return claim


def seal_speed_claim(report: dict) -> dict:
    """Make the speed TOKEN and the speed FIELDS agree, in both directions. Fail closed.

    STRUCTURAL, AND IT DELETES. :func:`seal_verdict` may downgrade a token and leave the
    numbers on the record, because those numbers are a decomposition of one run and a
    reader who meets the withholding can still check them. These two are different: they
    are a claim *about another provider*, and a reader who sees
    ``speedup_vulkan_over_cuda: 9.0`` has already been told the thing the refusal was
    supposed to prevent.

    Applied on every serialisation through :func:`dump_profile_json`, so a report that
    acquires the fields anywhere — a fresh run, a copied-forward reanalysis, a future
    caller — cannot carry them past the boundary without a claim that stands behind them.
    A report with no claim at all is refused for the same reason an empty equivalence map
    is: absence of a verdict is not a verdict.

    THE OTHER DIRECTION, ADDED AFTER REVIEW OF THE ISSUE #69 BRANCH.  The seal only ever
    asked "is the token green?" and deleted the fields when it was not. It never asked the
    converse, and the converse was reachable: :func:`attribute` returned at
    ``TRACE_ABSENT`` *before* it recorded ``untraced_median_ms``, so on ``--reanalyse`` of
    a record whose trace cannot be read, :func:`publish_speed_claim` found no medians to
    divide, wrote no fields, and left a claim reading ``SPEED_COMPARED`` with
    ``fields: [gap_ms, speedup_vulkan_over_cuda]`` and neither field on the record. A
    token that says a comparison was made, listing by name two numbers that are not there,
    is exactly the shape a reader quotes and cannot check.

    So the two sets are now made consistent structurally: a ``SPEED_COMPARED`` claim whose
    fields are not ALL present and numeric is downgraded to ``SPEED_REFUSED`` naming what
    was missing, and the partial fields are then deleted with the rest. Either there is no
    speed comparison token, or the comparison is complete. Never a token without numbers.
    """
    claim = report.get("speed_claim") or {}
    if claim.get("verdict") == SPEED_COMPARED:
        missing = [k for k in SPEED_FIELDS
                   if not isinstance(report.get(k), (int, float))
                   or isinstance(report.get(k), bool)]
        if missing:
            claim["verdict"] = SPEED_REFUSED
            claim.setdefault("refusals", []).append(
                f"the claim was SPEED_COMPARED and the report carries no numeric "
                f"{', '.join(missing)}. The equivalence contract was met and the medians "
                f"needed to derive the field(s) were not on the record, so there is no "
                f"comparison to publish — a token naming fields that are absent is a "
                f"claim a reader cannot check")
            claim["incomplete_fields"] = list(missing)
            report["speed_claim"] = claim
    if claim.get("verdict") != SPEED_COMPARED:
...
def dump_profile_json(report: dict, out_path) -> str:
    """The ONLY serialiser this module uses. Both gates, then the boundary.

    A single funnel rather than a rule to remember: `test_no_report_is_serialised_without
    _both_seals` reads this module's source and fails if a `dump_public_json` call appears
    anywhere else in it. The alternative — "call seal_speed_claim before you write" — is
    the obligation that produced the defect in the first place.
    """
    seal_verdict(report)
    seal_speed_claim(report)
    return dump_public_json(report, out_path)
...
def add_op_comparison(report: dict, untraced: "dict | None", cuda: "dict | None") -> dict:
    """Per-op-type comparison from the two arms' ORT profiles, or the reason there is none.

    Extracted so the fresh run and ``--reanalyse`` cannot derive it two different ways.
    Reanalysis used to COPY the stored table forward, which meant a table produced under
    one equivalence contract could be republished under another without ever being
    re-derived; the handles are on the record, so it is recomputed from them instead.
    """
    report.pop("op_comparison", None)
    report.pop("op_comparison_refused", None)
    if not untraced or not cuda:
        report["op_comparison_refused"] = (
            "one of the two arms was not dispatched, so there are no two profiles to "
            "compare; an absent arm is not an arm with no shared op types")
        return report
    cuda_prof, cuda_why = cc.resolve_scratch_file(cuda, cuda.get("profile_rel"))
    vk_prof, vk_why = cc.resolve_scratch_file(untraced, untraced.get("profile_rel"))
    if cuda_prof and vk_prof:
        vk_ops = op_kernel_times(vk_prof)
        cuda_ops = op_kernel_times(cuda_prof)
        if vk_ops and cuda_ops:
            report["op_comparison"] = op_type_comparison(vk_ops, cuda_ops)
        else:
            # An op comparison built from an unreadable profile is an empty table,
            # and an empty table reads as "the two providers share no op types" —
            # a finding — rather than as "this file was never parsed". Say which.
            empty = [name for name, ops in (("vulkan", vk_ops), ("cuda", cuda_ops))
                     if not ops]
            report["op_comparison_refused"] = (
                f"the {', '.join(empty)} profile parsed to zero kernel-time events, so "
                f"there is no per-op comparison; an empty comparison is not a finding "
                f"about either provider's op coverage")
    else:
        report["op_comparison_refused"] = "; ".join(
            w for w in (vk_why and f"vulkan profile: {vk_why}",
                        cuda_why and f"cuda profile: {cuda_why}") if w)
    return report


def publish_speed_claim(report: dict, *, untraced: "dict | None", cuda: "dict | None",
                        reference: "dict | None" = None) -> dict:
    """Derive the gated speed fields, or make sure the report carries none.

    The ONLY writer of :data:`SPEED_FIELDS`. Both are recomputed here from the medians on
    the record and are set only when :func:`speed_claim` says ``SPEED_COMPARED``; the
    final :func:`seal_speed_claim` is what turns "was not set" into "cannot survive",
    which is the difference between this being a rule and being a mechanism.
    """
    claim = speed_claim(untraced, cuda, reference)
    report["speed_claim"] = claim
    vk_med = report.get("untraced_median_ms")
    cu_med = report.get("cuda_median_ms")
    if claim["verdict"] == SPEED_COMPARED and vk_med and cu_med:
        report["gap_ms"] = vk_med - cu_med
        report["speedup_vulkan_over_cuda"] = cu_med / vk_med
    return seal_speed_claim(report)


def compare_against_cuda(report: dict, *, untraced: "dict | None", cuda: "dict | None",
                         reference: "dict | None" = None) -> dict:
    """The whole cross-arm reduction: the op table, then the gated speed claim.

    One function for both entry points, because the two used to disagree: the fresh run
    derived these fields under the equivalence contract and ``--reanalyse`` copied them
    forward under none.
    """
    add_op_comparison(report, untraced, cuda)
    return publish_speed_claim(report, untraced=untraced, cuda=cuda, reference=reference)
    ...
    try:
        ...
        for k, v in prev.items():
            if v is None:
                ...
                os.environ[k] = v
    # Two fields, one directory, and the distinction is the same one `dispatch_arm`
    # draws: `trace_path` is *evidence* — rooted, machine-independent, an answer to
    # "which file was this?" — and nothing opens it. `trace_rel` is the *handle*, and
    # `resolve_trace` is the only thing allowed to turn it into a file.
    rec["trace_path"] = cc.public_path(trace_path)
    rec["trace_rel"], why = cc.relative_handle(trace_path, scratch)
    if rec["trace_rel"] is None:
        rec["trace_rel_absent_because"] = why
    return rec


def resolve_trace(rec: dict) -> "tuple[Path | None, str]":
    """The EP trace this record names, or why it cannot be read.

    Shares `cuda_competition.resolve_scratch_file` with the output-tensor path on
    purpose: one contract for every file that crosses a process boundary in this suite,
    so a second reader cannot be written that resolves handles a third way.
    """
    return cc.resolve_scratch_file(rec, rec.get("trace_rel"))


def attribute(traced: dict, untraced: dict | None, trace_path: "Path | None",
              *, unresolved: str = "") -> dict:
    """Reduce one traced Vulkan run into a cause attribution.

    ``trace_path`` may be ``None``, and ``unresolved`` is then the sentence explaining
    why. That pairing is deliberate: the caller resolves the trace handle through
    `resolve_trace`, which refuses rather than guesses, and a refusal must arrive here as
    a reason rather than as a path that merely fails to exist. The two are different
    findings — "the EP wrote no trace" and "this record's handle cannot be resolved on
    this machine" — and collapsing them once let a rooted, unopenable path be reported as
    an absent trace.
    """
    out: dict = {
        "arm": cc.ARM_VULKAN,
    ...
    # THE MEDIANS ARE RECORDED BEFORE ANY EXIT, NOT AFTER THE LAST ONE.  They came off the
    # dispatched records and cost nothing to state; they are not a conclusion of the trace
    # reduction, and they were sitting below an early return that a missing or unresolvable
    # trace takes. `publish_speed_claim` derives the cross-arm fields FROM them, so on
    # `--reanalyse` of such a record the equivalence contract could be met with no numbers
    # to divide, and the report went out carrying `SPEED_COMPARED` naming two fields that
    # were not on it. The seal now refuses that shape outright; recording the medians here
    # is the other half — the honest record keeps its comparison instead of losing it to an
    # ordering accident.
...
def render(report: dict) -> str:
    ...
    # has to arrive before the numbers, not in a REFUSAL footnote under them. The speed
    # claim is sealed here for the same reason and one step harder: a refused cross-arm
    # ratio is not printed at all, because the markdown is what gets pasted into a PR.
    seal_verdict(report)
    seal_speed_claim(report)
    ...
    claim = report.get("speed_claim") or {}
    if claim.get("verdict") == SPEED_REFUSED:
        lines.append("")
        lines.append(f"> **No Vulkan-vs-CUDA speed result is published.** "
                     f"`{'`, `'.join(SPEED_FIELDS)}` are absent from this report, not "
                     f"relabelled, because the arms they would compare are not both "
                     f"admissible and output-equivalent:")
        for why in claim.get("refusals") or []:
            lines.append(f"> - {why}")
    lines.append("")
...
def main(argv=None) -> int:
    ...
    if a.reanalyse:
        ...
        trace_file, why = resolve_trace(traced)
        report = attribute(traced, untraced, trace_file, unresolved=why)
        # EVIDENCE IS COPIED FORWARD; CONCLUSIONS ARE RE-DERIVED.  `op_comparison`,
        # `gap_ms` and `speedup_vulkan_over_cuda` used to be on this list, which meant a
        # reanalysis whose equivalence check refused still republished the speed fields the
        # original run had derived under a contract this one has just declined to honour.
        # A refusal that leaves the number in the file has refused nothing.
        for k in ("schema", "iters", "warmup", "untraced_record", "traced_record",
                  "cuda_record", "cpu_reference_record", "cuda_median_ms"):
            if k in prior:
                report[k] = prior[k]
        cuda = prior.get("cuda_record")
        if cuda or untraced:
            compare_against_cuda(report, untraced=untraced, cuda=cuda,
                                 reference=prior.get("cpu_reference_record"))
        ...
        dump_profile_json(report, out_path)
        md = render(report)
    ...
    if not a.skip_cuda:
        ...
        # THE EQUIVALENCE REFERENCE, DISPATCHED BECAUSE THE CLAIM NEEDS ONE.  A
        # Vulkan-vs-CUDA ratio says the two arms computed the same thing at different
        # speeds, and nothing in this module checked that half of the sentence. The arm
        # and the reference are `cuda_competition`'s, not a second set: `speed_claim`
        # hands all three records to `cross_arm_equivalence` exactly as the suite does.
        # It is dispatched only on the path that can make a cross-arm claim — with
        # `--skip-cuda` there is no ratio, so there is nothing to check.
        print(f"[profile] equivalence reference: {SPEED_REFERENCE_ARM}/{workload.key}",
              file=sys.stderr)
        reference = cc.dispatch_arm(SPEED_REFERENCE_ARM, workload, iters=a.iters,
                                    warmup=a.warmup, scratch=scratch / "reference",
                                    device=a.device, seed=a.seed)
        report["cpu_reference_record"] = reference
        compare_against_cuda(report, untraced=untraced, cuda=cuda, reference=reference)
