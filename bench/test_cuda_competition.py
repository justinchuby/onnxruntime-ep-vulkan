...
# ---------------------------------------------------------------------------
# B3: EQUIVALENCE COVERS EVERY REPEAT WHOSE SAMPLES DECIDE THE CLAIM
#
# Gate 4 was closed for repeat 0 and left open for every other repeat. `run_suite`
# compared `[r for r in per_workload if r.get("repeat") == 0]` and handed the result to
# `compare_workload`, which pooled `steady_ms` across ALL repeats into the ratio. So an
# arm only had to compute the right answer once to have every repeat's timings counted,
# and the tensors of every later repeat were never opened.
#
# The controls below plant exactly that: correct on repeat 0, a thousandfold wrong on
# repeat 1, nine times faster on both. Each runs through the real `dump_public_json`, and
# the last is the must-pass polarity — a multi-repeat arm that agrees on every repeat is
# still compared and is still allowed to win.
# ---------------------------------------------------------------------------

def _repeat_records(scratch: Path, per_repeat_vulkan_logits) -> "list[dict]":
    """Three arms x N repeats, each repeat in its own slot, every record through the boundary."""
    records = []
    for rep, vulkan_logits in enumerate(per_repeat_vulkan_logits):
        rep_scratch = scratch / f"rep{rep}"
        rep_scratch.mkdir(parents=True, exist_ok=True)
        for arm, logits, steady in (
            (cc.ARM_CPU_HOST, _TRUE_LOGITS, [50.0] * 5),
            (cc.ARM_VULKAN, vulkan_logits, _FAST_VULKAN),
            (cc.ARM_CUDA, _TRUE_LOGITS, _SLOW_CUDA),
        ):
            rec = _worker_record(rep_scratch, arm, logits, steady)
            rec["repeat"] = rep
            records.append(rec)
    return records


def _suite_through_the_boundary(tmp_path, records) -> dict:
    """One workload's suite, serialised and read back the way a reviewer receives it."""
    w = _workload()
    eq = cc.workload_equivalence(records)
    suite = {"schema": "cuda_competition/1", "iters": 20, "warmup": 5, "repeats": 2,
             "results": records, "equivalence": {w.key: eq},
             "comparisons": [cc.compare_workload(w, records, equivalence=eq)]}
    out = tmp_path / "suite.json"
    pp.dump_public_json(suite, out)
    return json.loads(out.read_text(encoding="utf-8"))


def test_a_wrong_second_repeat_cannot_publish_vulkan_faster(tmp_path):
    """The defect, end to end: right on repeat 0, a thousandfold wrong on repeat 1.

    Before this, equivalence looked at repeat 0 alone and `compare_workload` pooled both
    repeats' samples, so this arm published VULKAN_FASTER on timings half of which came
    from a run whose outputs nothing had read.
    """
    records = _repeat_records(tmp_path, [_TRUE_LOGITS, _WRONG_BY_1000])
    published = _suite_through_the_boundary(tmp_path, records)

    eq = published["equivalence"][_workload().key]
    cmp_ = published["comparisons"][0]

    assert eq["verdict"] == cc.EQ_REFUSED
    assert eq["repeats"]["0"]["arms"][cc.ARM_VULKAN]["verdict"] == "MATCH"
    assert eq["repeats"]["1"]["arms"][cc.ARM_VULKAN]["verdict"] == "DIVERGENT"
    assert eq["arms"][cc.ARM_VULKAN]["verdict"] == "DIVERGENT"
    assert eq["arms"][cc.ARM_VULKAN]["repeats"] == {"0": "MATCH", "1": "DIVERGENT"}

    assert cmp_["verdict"] == cc.NOT_EQUIVALENT
    assert cmp_["verdict"] != "VULKAN_FASTER"
    assert "speedup_vulkan_over_cuda" not in cmp_
    assert cmp_["equivalence_divergent_repeats"] == [f"{cc.ARM_VULKAN}@repeat1=DIVERGENT"]
    assert cmp_["contributing_repeats"][cc.ARM_VULKAN] == {"0": 5, "1": 5}, (
        "the samples this control is about must actually be in the pool, or it proves "
        "nothing")


def test_an_arm_equivalent_on_every_repeat_still_compares_and_can_win(tmp_path):
    """THE MUST-PASS POLARITY. A screen that refuses every multi-repeat run protects nothing."""
    records = _repeat_records(tmp_path, [_TRUE_LOGITS, _TRUE_LOGITS])
    published = _suite_through_the_boundary(tmp_path, records)

    eq = published["equivalence"][_workload().key]
    cmp_ = published["comparisons"][0]

    assert eq["verdict"] == cc.EQ_COMPARED, eq.get("detail")
    assert eq["arms"][cc.ARM_VULKAN]["repeats"] == {"0": "MATCH", "1": "MATCH"}
    assert eq["compared_arms"] == sorted([cc.ARM_VULKAN, cc.ARM_CUDA])
    assert cmp_["equivalence_divergent_repeats"] == []
    assert cmp_["equivalence_unchecked_repeats"] == []
    assert cmp_["verdict"] == "VULKAN_FASTER"
    assert cmp_["n_vulkan"] == 10, "both repeats' samples are in the ratio"


def test_a_repeat_whose_samples_count_and_whose_tensors_were_never_read_is_refused(tmp_path):
    """The exact shape of the old bug, planted at the equivalence result rather than the tensor.

    `workload_equivalence` is replaced by the thing it superseded: a comparison of repeat 0
    only. Every arm MATCHes, and repeat 1's samples are still in the pool — so the pooled
    ratio rests on evidence nothing checked, and `compare_workload` must say so instead of
    publishing.
    """
    records = _repeat_records(tmp_path, [_TRUE_LOGITS, _WRONG_BY_1000])
    rep0_only = cc.cross_arm_equivalence([r for r in records if r["repeat"] == 0])
    assert rep0_only["verdict"] == cc.EQ_COMPARED, "the fixture must reproduce the old pass"

    entry = cc.compare_workload(_workload(), records,
                                equivalence={"repeats": {"0": rep0_only},
                                             "arms": rep0_only["arms"],
                                             "verdict": rep0_only["verdict"]})

    assert entry["verdict"] == cc.INSTRUMENT_ERROR
    assert entry["verdict"] != "VULKAN_FASTER"
    assert "speedup_vulkan_over_cuda" not in entry
    assert entry["equivalence_unchecked_repeats"] == sorted(
        [f"{cc.ARM_VULKAN}@repeat1", f"{cc.ARM_CUDA}@repeat1"])


def test_an_empty_per_repeat_verdict_is_a_refusal_not_a_pass(tmp_path):
    """The closure property, one level down: silence in a repeat is not consent for it."""
    records = _repeat_records(tmp_path, [_TRUE_LOGITS, _TRUE_LOGITS])
    eq = cc.workload_equivalence(records)
    eq["repeats"]["1"]["arms"][cc.ARM_VULKAN] = {"verdict": "", "outputs": []}
    eq["arms"][cc.ARM_VULKAN]["repeats"].pop("1")

    entry = cc.compare_workload(_workload(), records, equivalence=eq)

    assert entry["verdict"] == cc.INSTRUMENT_ERROR
    assert f"{cc.ARM_VULKAN}@repeat1" in entry["equivalence_unchecked_repeats"]


def test_the_fold_keeps_the_repeat_and_does_not_key_it_away_by_arm(tmp_path):
    """A fold that writes `arms[arm] = verdict` inside a loop keeps whichever repeat is last.

    Both orderings are asserted, because the defect is order-dependent by construction: a
    last-writer-wins fold looks correct on exactly one of these two fixtures.
    """
    for logits in ([_TRUE_LOGITS, _WRONG_BY_1000], [_WRONG_BY_1000, _TRUE_LOGITS]):
        scratch = tmp_path / ("a" if logits[0] is _TRUE_LOGITS else "b")
        scratch.mkdir()
        eq = cc.workload_equivalence(_repeat_records(scratch, logits))

        assert eq["arms"][cc.ARM_VULKAN]["verdict"] == "DIVERGENT", logits
        assert set(eq["arms"][cc.ARM_VULKAN]["repeats"]) == {"0", "1"}
        assert sorted(eq["arms"][cc.ARM_VULKAN]["repeats"].values()) == ["DIVERGENT", "MATCH"]
        assert eq["repeat_ids"] == ["0", "1"]


def test_the_rendered_report_names_the_repeat_each_verdict_belongs_to(tmp_path):
    """A folded verdict with no repeat in it is the sentence that hid repeat 1."""
    records = _repeat_records(tmp_path, [_TRUE_LOGITS, _WRONG_BY_1000])
    w = _workload()
    eq = cc.workload_equivalence(records)
    md = cc.render({"iters": 20, "warmup": 5, "repeats": 2, "device_facts": {},
                    "results": records, "equivalence": {w.key: eq},
                    "comparisons": [cc.compare_workload(w, records, equivalence=eq)]})

    assert "| workload | repeat | arm |" in md
    assert f"| {w.key} | 1 | {cc.ARM_VULKAN} | DIVERGENT" in md
    assert f"| {w.key} | 0 | {cc.ARM_VULKAN} | MATCH" in md
    assert "repeat 0=MATCH, repeat 1=DIVERGENT" in md


def test_a_single_repeat_suite_is_unchanged_by_the_repeat_contract(tmp_path):
    """The compatibility polarity: one repeat, and everything reads as it did."""
    records = _repeat_records(tmp_path, [_TRUE_LOGITS])
    eq = cc.workload_equivalence(records)
    entry = cc.compare_workload(_workload(), records, equivalence=eq)

    assert eq["verdict"] == cc.EQ_COMPARED
    assert eq["arms"][cc.ARM_VULKAN]["repeats"] == {"0": "MATCH"}
    assert entry["verdict"] == "VULKAN_FASTER"


def test_run_suite_and_reanalyse_both_use_the_per_repeat_contract():
    """Structural: neither entry point may go back to comparing one repeat.

    Read from the source, because the failure is a *call site* — `run_suite` and
    `reanalyse` each selected repeat 0 by hand, and no behavioural test of
    `workload_equivalence` can see a caller that stopped using it.
    """
    for fn in (cc.run_suite, cc.reanalyse):
        src = inspect.getsource(fn)
        assert "workload_equivalence(" in src, (
            f"{fn.__name__} does not fold equivalence over every repeat")
        assert "cross_arm_equivalence(" not in src, (
            f"{fn.__name__} calls cross_arm_equivalence directly; that is the call that "
            f"compared one repeat while the ratio pooled all of them")
        assert 'repeat") == 0' not in src and 'repeat", 0) == 0' not in src


def test_workload_equivalence_over_no_records_is_a_refusal():
    """Zero repeats is not "every repeat agreed"."""
    eq = cc.workload_equivalence([])

    assert eq["verdict"] == cc.EQ_REFUSED
    assert eq["arms"] == {} and eq["repeats"] == {}
    assert "refusal" in eq["detail"]

# ---------------------------------------------------------------------------
# B3b: A REPEAT-BLIND EQUIVALENCE MAP MAY SPEAK FOR AT MOST ONE REPEAT
#
# The per-repeat contract above closed the defect for a result that KNOWS about repeats.
# It left one door open, labelled compatibility: a flat `{arm: verdict}` map — a
# single-repeat run, or any suite stored before the contract existed — was honoured for the
# arms it named, whatever the number of repeats those arms contributed.
#
# That is the original defect with one layer of indirection. Under the three-repeat
# protocol (§6.6 of docs/PERF.md) an arm-only verdict was standing in for three separate
# comparisons of three separate output sets, and a divergent second repeat was invisible
# again — this time without even a `repeats` key to notice was missing.
#
# So the path is CONSTRAINED, not deleted: honoured exactly where it cannot be wrong (an
# arm contributing one repeat, which is what produced it) and refused by name everywhere
# else. Both polarities, because a compatibility path that refuses the single-repeat case
# breaks every stored suite for nothing.
# ---------------------------------------------------------------------------

def _flat_equivalence(verdict: str = "MATCH") -> dict:
    """A repeat-blind result in the shape a pre-contract suite stored."""
    return {"verdict": cc.EQ_COMPARED,
            "arms": {cc.ARM_VULKAN: {"verdict": verdict},
                     cc.ARM_CUDA: {"verdict": verdict}}}


def test_a_flat_equivalence_map_may_not_speak_for_more_than_one_repeat(tmp_path):
    """NEGATIVE POLARITY: two repeats, one arm-only verdict, refused by arm and repeat."""
    records = _repeat_records(tmp_path, [_TRUE_LOGITS, _WRONG_BY_1000])

    entry = cc.compare_workload(_workload(), records, equivalence=_flat_equivalence())

    assert entry["verdict"] == cc.INSTRUMENT_ERROR, entry
    assert entry["equivalence_unchecked_repeats"] == sorted(
        [f"{cc.ARM_VULKAN}@repeat0", f"{cc.ARM_VULKAN}@repeat1",
         f"{cc.ARM_CUDA}@repeat0", f"{cc.ARM_CUDA}@repeat1"]), entry
    assert "speedup_vulkan_over_cuda" not in entry, (
        "a ratio was published from samples no comparison spoke for")


def test_a_flat_equivalence_map_still_speaks_for_a_single_repeat(tmp_path):
    """MUST-PASS POLARITY: the case the compatibility path exists for still works.

    A stored single-repeat suite carries no `repeats` key at all. Refusing it would break
    every artifact predating the contract to close a hole that shape cannot have.
    """
    records = _repeat_records(tmp_path, [_TRUE_LOGITS])

    entry = cc.compare_workload(_workload(), records, equivalence=_flat_equivalence())

    assert entry["equivalence_unchecked_repeats"] == [], entry
    assert entry["equivalence_unchecked_arms"] == []
    assert entry["verdict"] == "VULKAN_FASTER", entry


def test_a_flat_map_that_disqualifies_a_single_repeat_arm_is_still_honoured(tmp_path):
    """The compatibility path must carry a REFUSAL forward too, not only an acquittal."""
    records = _repeat_records(tmp_path, [_TRUE_LOGITS])

    entry = cc.compare_workload(_workload(), records,
                                equivalence=_flat_equivalence("DIVERGENT"))

    assert entry["verdict"] == cc.NOT_EQUIVALENT, entry
    assert cc.ARM_VULKAN in entry["equivalence_disqualified_arms"]


def test_the_multi_repeat_refusal_is_about_the_map_and_not_the_arm(tmp_path):
    """One arm with one repeat and one with two, under the same flat map.

    The constraint is per (arm, repeat) like everything else here: an arm the flat map
    genuinely can speak for is not punished for another arm's repeat count.
    """
    records = [r for r in _repeat_records(tmp_path, [_TRUE_LOGITS, _TRUE_LOGITS])
               if r["arm"] != cc.ARM_CUDA or r["repeat"] == 0]

    entry = cc.compare_workload(_workload(), records, equivalence=_flat_equivalence())

    assert entry["equivalence_unchecked_repeats"] == sorted(
        [f"{cc.ARM_VULKAN}@repeat0", f"{cc.ARM_VULKAN}@repeat1"]), entry
    assert f"{cc.ARM_CUDA}@repeat0" not in entry["equivalence_unchecked_repeats"]
