...
reported", and the summary says so in the same breath as the ratio.
...
#: ``baseline_median_ms / candidate_median_ms``. Above 1 means the candidate is faster. Stated
#: once, here, because a ratio whose direction is ambiguous has caused two rejections already.
...
def _process_key(record: Mapping[str, Any]) -> str:
    ...
    A bare PID is not one. Windows recycles PIDs, and these records span two sessions on
    different days, so the same integer genuinely appears in both — the treatment run and the
    calibration run each happened to get PID 22100. Pairing the PID with the process's own start
    timestamp gives an identity that is stable across sessions and still catches the case the
    contamination check is actually for: the same process appearing on both sides. The result is
    a string so that it survives a JSON round-trip unchanged.
    """
...
def calibration(aa_pairs: Sequence[Mapping[str, Any]], *, treatment_keys: Iterable[Any] = ()) -> dict:
    ...
    it is the only honest scale against which a build difference can be called real.
...
def verdict_for(pair: Mapping[str, Any], band: "float | None", *, repeats_required: int) -> dict:
    ...
    ``SLOWER`` requires *every* repeat below ``1 - band``; ``FASTER``, every repeat above
    ``1 + band``; ``NEUTRAL``, every repeat inside. Anything else — including a missing band, a
    short repeat count, or repeats that straddle — is ``INDETERMINATE``, with the reason named.
