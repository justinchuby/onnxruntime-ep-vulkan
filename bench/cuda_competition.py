...
import public_paths  # noqa: E402
from public_paths import (  # noqa: E402
    RUNTIME_ONLY_KEY, dump_public_json, public_path, write_public_text)
...
#: whole island with one fused node.  On Phi-3.5 it claimed `claimed_nodes` 355 of
#: `total_nodes_probed` 363 probed nodes into a single island -- so 97.8% of the probed
#: graph by `claimed_nodes` over `total_nodes_probed`, and `island_count` 1, all
#: committed in ``bench/results/barrier_ab-post-dev0-0.json``.  A node-counting reader
#: sees that island as *one* provider node, so the same run reads as overwhelmingly GPU
#: by claimed nodes -- `claimed_nodes` 355 over `total_nodes_probed` 363, 97.8% of the
#: probed graph, committed above. That is the one number this
...
# ---------------------------------------------------------------------------
# Cross-process handles: the boundary between what one process hands another
# and what this suite is allowed to publish
# ---------------------------------------------------------------------------

#: The only file name `_npy_dump` writes a full tensor to, and therefore the only one
#: `resolve_arm_output` will open. Named as a constant because writer and reader must
#: agree by construction rather than by two string literals that happen to match.
OUTPUT_HANDLE = "out0.npy"

#: Manifest fields that mean "an absolute path this process can open". A record carrying
#: one was written by a worker predating the relative-handle contract, and it is refused
#: rather than read: after `dump_public_json` such a field holds a *rooted* path
#: (``<repo>/…``) that opens nothing, and treating it as a path that merely happens to be
#: missing is precisely how a wrong arm went unchecked. See :func:`_npy_dump`.
_LEGACY_ABS_HANDLE = ("file", "path")


def _handle_base(rec: dict, key: str) -> "tuple[Path | None, str]":
    """The directory a relative handle in *rec* is rooted at, or why there is none.

    Two sources, in the order that keeps a live run and a re-analysis honest:

    1. ``rec[RUNTIME_ONLY_KEY][key]`` — the absolute directory the parent passed to the
       worker on the command line. It exists only while the run is in memory, because
       `public_payload` drops it, so it cannot be the thing a committed artifact leans on.
    2. ``rec[key]`` — the same directory as a *rooted* path in the published record,
       turned back into a real directory by `public_paths.resolve_public_path`. This is
       what lets ``--reanalyse`` re-derive equivalence from a committed record on a
       second machine.

    An absolute path in position 2 is refused, not used: a published record is not
    allowed to contain one, so honouring it would let the reader accept exactly what the
    writer is forbidden to emit.
    """
    runtime = rec.get(RUNTIME_ONLY_KEY) or {}
    raw = runtime.get(key)
    if raw:
        base = Path(raw)
        if not base.is_absolute():
            return None, (f"the in-process {key!r} handle {raw!r} is not absolute; the "
                          f"parent records the directory it dispatched, not a guess")
        return base, ""

    published = rec.get(key)
    if not published:
        return None, (f"the record carries no {key!r}, so there is no directory to root "
                      f"its output handles at")
    if Path(str(published)).is_absolute():
        return None, (f"{key!r} is an absolute path in a record that has been through "
                      f"serialisation; committed records carry rooted paths, and an "
                      f"absolute one here means the boundary was bypassed")
    resolved = public_paths.resolve_public_path(published)
    if resolved is None:
        return None, (f"{key!r} is {published!r}, which this machine cannot resolve to a "
                      f"directory; the outputs it names are not readable here and the "
                      f"comparison is UNMEASURED rather than passing")
    return resolved, ""


def relative_handle(target, base) -> "tuple[str | None, str]":
    """*target* named relative to *base*, or ``(None, why)`` when it is not inside it.

    The writer's half of the contract in :func:`resolve_arm_output`. A worker calls this
    on a path it just wrote and puts the result in its record.

    Total, and it returns its refusal rather than raising it, for the reason
    `bench/_polarity.py` sets out: the caller records the ``why`` beside the absent
    handle, so a reader can tell "no file was written" from "a file was written
    somewhere this record cannot name". Both polarities are asserted through
    `_polarity.refuses`/a plain call in `bench/test_cuda_competition.py`.

    THE BASE RULE IS THE READER'S, NOT A SECOND ONE.  This resolved an empty base to the
    process's current directory and wrote a handle against it, while
    `public_paths.contained_child` — the only thing allowed to read that handle back —
    refuses an empty base outright. A writer that can produce a name no reader will accept
    puts an unreadable handle in a committed record and spells it exactly like a readable
    one. Both halves now ask :func:`public_paths.unusable_base`, so the two cannot drift
    apart again; `bench/test_public_paths.py` asserts both polarities of both halves over
    the same table of bases.
    """
    unusable = public_paths.unusable_base(base)
    if unusable is not None:
        return None, (f"no handle can be written: {unusable}. The reader refuses this "
                      f"base too, and a writer that accepts one the reader will not is "
                      f"how a record acquires a handle nobody can open")
    if not target:
        return None, ("no path was produced, so there is nothing for a later process to "
                      "open")
    try:
        resolved = Path(str(target)).resolve()
        root = Path(base).resolve()
    except (OSError, ValueError, TypeError):
        # `ValueError` and not only `OSError`: `Path.resolve` raises it for an embedded
        # NUL, and `TypeError` for something that is not a path at all. A writer's half
        # that raises is a writer's half whose caller cannot record the `why` beside the
        # absent handle, which is the whole shape this function is built in.
        return None, (f"neither {target!r} nor {base!r} could be resolved on this machine, "
                      f"so no handle can be written for it")
    rel = public_paths._relative_to(resolved, root)
    if rel is None or str(rel) in ("", "."):
        return None, (f"the file is not inside the directory this record roots its handles "
                      f"at, so no handle can name it without an absolute path")
    handle = rel.as_posix()
    if public_paths.contained_child(root, handle) is None:
        # The writer and the reader share one rule. A name this process can produce but
        # `contained_child` will refuse — a trailing dot, a colon, a device name — must not
        # be written as a handle at all: the record would carry a handle that resolves for
        # nobody, and "the file was never written" and "the handle cannot be read" are
        # different findings.
        return None, (f"the file is inside the directory but {handle!r} is not a handle "
                      f"this suite's resolver will accept, so recording it would name a "
                      f"file no reader can open")
    return handle, ""


def resolve_scratch_file(rec: dict, rel: "str | None") -> "tuple[Path | None, str]":
    """Resolve a handle rooted at the arm's scratch directory, with containment.

    Same contract as :func:`resolve_arm_output`, for the files that live beside the
    record rather than in the per-arm output slot: the ORT profile and the EP trace.
    `cuda_profile` reads both, and before this existed it read them by calling
    ``Path(rec["profile_path"])`` on a field the serialiser had already rewritten —
    which produced an empty op comparison that looked like "the providers share no op
    types" instead of "this file was never opened".
    """
    if not rel:
        return None, "the record carries no handle for this file, so it was not written"
    base, why = _handle_base(rec, "scratch_dir")
    if base is None:
        return None, why
    target = public_paths.contained_child(base, rel)
    if target is None:
        return None, (f"handle {rel!r} does not resolve to a file inside this arm's own "
                      f"scratch directory; a handle that escapes it is refused")
    if not target.is_file():
        return None, f"no file at handle {rel!r} under this arm's scratch directory"
    return target, ""


def resolve_arm_output(rec: dict, entry: dict) -> "tuple[Path | None, str]":
    """Turn one manifest entry into a readable tensor file, or say why it is not one.

    Fail-closed by construction: every return that is not a file is a *sentence*, and
    :func:`cross_arm_equivalence` turns every such sentence into a verdict that
    disqualifies the arm. There is no path through this function that leaves a caller
    unable to distinguish "compared and agreed" from "could not be compared".
    """
    for legacy in _LEGACY_ABS_HANDLE:
        if entry.get(legacy):
            return None, (
                f"manifest entry names {legacy!r}={entry[legacy]!r} instead of a relative "
                f"{'file_rel'!r} handle. That field is either an absolute path (which a "
                f"committed record may not carry) or a rooted one (which opens nothing), "
                f"so it is refused rather than read as a missing file")
    rel = entry.get("file_rel")
    if not rel:
        return None, "manifest entry carries no file_rel handle, so no tensor was written"
    if rel != OUTPUT_HANDLE:
        return None, (f"manifest handle {rel!r} is not {OUTPUT_HANDLE!r}, the only name "
                      f"this module writes a full tensor to")
    base, why = _handle_base(rec, "outputs_dir")
    if base is None:
        return None, why
    target = public_paths.contained_child(base, rel)
    if target is None:
        return None, (f"handle {rel!r} does not resolve to a file inside this arm's own "
                      f"output directory; a handle that escapes its directory is refused")
    if not target.is_file():
        return None, (f"this arm's {rel!r} is not on disk under its output directory, so "
...
def _npy_dump(outputs, names, path: Path) -> "list[dict]":
    ...
    THE HANDLE IS RELATIVE, AND THAT IS THE FIX FOR GATE 4.  This ran in a worker
    process and wrote ``entry["file"] = str(<absolute path>)``. The record then went
    through `dump_public_json`, which correctly refuses to publish an absolute path and
    rewrote it ``<repo>/…`` — so the parent, reading the record back, asked
    ``Path("<repo>/…").is_file()``, got ``False``, and reported the reference output as
    missing. `compare_workload` saw an empty ``arms`` map, concluded that no arm had
    been disqualified, and published ``VULKAN_FASTER`` for an arm whose logits were
    wrong by a factor of a thousand. The sanitiser was not the bug; using one field for
    both the private channel and the public artifact was.

    So the manifest names the file *within the arm's own output directory* and nothing
    else. It survives sanitisation unchanged because there is nothing in it to sanitise,
    and the parent turns it back into a file with
    :func:`resolve_arm_output`, which rejects anything that does not stay inside that
    directory. ``file`` is not written any more, and a record that still carries one is
    refused rather than read — see :data:`_LEGACY_ABS_HANDLE`.
    """
    import numpy as np
    ...
    for idx, (arr, name) in enumerate(zip(outputs, names)):
        ...
        if idx == 0:
            f = path / OUTPUT_HANDLE
            np.save(f, arr)
            entry["file_rel"] = OUTPUT_HANDLE
        else:
            import hashlib
...
def _worker_main(argv: "list[str]") -> int:
    ...
    # The profile the parent may want to reduce, named the way the parent can open it:
    # relative to the scratch directory the parent itself passed in. `profile_path` stays
    # in the record as the rooted, human-readable answer to "which file was this?", and
    # is never the field anything opens. See `resolve_scratch_file`.
    payload["profile_rel"], why = relative_handle(getattr(rec, "profile_path", ""), scratch)
    if payload["profile_rel"] is None:
        payload["profile_rel_absent_because"] = why
...
#: The two scratch slots a dispatched arm writes into. Both are named for the arm and the
#: workload and for nothing else — no pid, no timestamp — so a second run of the same arm
#: on the same workload lands on the same two paths. That is deliberate (a reader can find
#: the record for an arm without hunting), and it is exactly why `_clear_stale_arm_scratch`
#: exists: a slot that is reused is a slot that can still hold the previous run's answer.
_ARM_RESULT_SLOT = re.compile(r"^result_[A-Za-z0-9][A-Za-z0-9_.+-]*\.json$")
_ARM_OUTPUT_SLOT = re.compile(r"^outputs_[A-Za-z0-9][A-Za-z0-9_.+-]*$")


def _clear_stale_arm_scratch(out: Path, dump: Path, scratch: Path) -> "list[str]":
    """Empty this arm's two scratch slots before its worker starts. Returns refusals.

    WHY THIS IS NOT HOUSEKEEPING.  `dispatch_arm` decides "the worker did not write a
    record" by asking whether ``out`` is a file, and `cross_arm_equivalence` compares the
    ``out0.npy`` named in the record's manifest. Both questions are asked *after* a
    subprocess that is allowed to die — segfault in a CUDA DLL, `TerminateProcess`, a
    machine that reboots. If the previous run of the same arm on the same workload left an
    ``ADMISSIBLE`` record and a tensor in these fixed paths, a crashing worker reads as a
    clean success and the equivalence check compares yesterday's numbers with today's. The
    verdict would be wrong in the safest-looking direction, which is the one nobody checks.

    `run_arm` already does this for its two per-run logs (``claim_log.unlink``,
    ``counters_file.unlink``); this is the same discipline one level up, applied to the
    two slots the parent process actually reads.

    THE DELETION IS NARROW ON PURPOSE.  ``dump`` is removed only when it resolves to a
    direct child of *scratch*, its name matches the ``outputs_<arm>_<workload>`` slot
    shape, and it is a real directory rather than a symlink — three conditions, all
    checked after `resolve()`, so a symlinked or relocated slot is refused rather than
    followed. Entries inside it are unlinked one by one instead of `rmtree`'d, and an
    unexpected sub-directory is reported rather than removed: this function may empty a
    slot this module writes, and it may not delete a tree.

    A refusal here is an ``INSTRUMENT_ERROR`` for the arm, never a silent continue. Running
    the worker with a slot this function could not clear is precisely the state it exists
    to prevent.
    """
    problems: "list[str]" = []
    try:
        scratch_resolved = scratch.resolve()
    except OSError as exc:  # pragma: no cover - unresolvable scratch is refused, not guessed
        return [f"scratch directory {scratch} could not be resolved: {exc!r}"]

    if not _ARM_RESULT_SLOT.match(out.name):
        problems.append(f"refusing to clear {out.name!r}: not a result slot this module names")
    else:
        try:
            if out.resolve().parent != scratch_resolved:
                problems.append(
                    f"refusing to clear {out}: it does not resolve to a child of {scratch}")
            else:
                out.unlink(missing_ok=True)
        except OSError as exc:
            problems.append(f"stale record {out} could not be removed: {exc!r}")

    if not dump.exists():
        return problems
    if not _ARM_OUTPUT_SLOT.match(dump.name):
        problems.append(f"refusing to clear {dump.name!r}: not an output slot this module names")
        return problems
    if dump.is_symlink() or not dump.is_dir():
        problems.append(f"refusing to clear {dump}: it is not a real directory")
        return problems
    try:
        if dump.resolve().parent != scratch_resolved:
            problems.append(
                f"refusing to clear {dump}: it does not resolve to a child of {scratch}")
            return problems
        for entry in sorted(dump.iterdir()):
            if entry.is_dir() and not entry.is_symlink():
                problems.append(
                    f"refusing to clear {entry}: this slot holds output files, and a "
                    f"sub-directory here was not written by this module")
                continue
            entry.unlink()
    except OSError as exc:
        problems.append(f"stale outputs in {dump} could not be removed: {exc!r}")
    return problems
...
def dispatch_arm(arm: str, workload: Workload, *, iters: int, warmup: int,
    ...
    # Before the worker starts, not after it fails: see `_clear_stale_arm_scratch`. A slot
    # that could not be emptied is an instrument error for this arm, because every question
    # asked below this line ("did it write a record?", "do the tensors agree?") would be
    # answered by the previous run instead.
    stale = _clear_stale_arm_scratch(out, dump, scratch)
    if stale:
        return {"arm": arm, "workload": workload.key, "model_key": workload.model_key,
                "verdict": INSTRUMENT_ERROR,
                "instrument_errors": [
                    "this arm's scratch slots still hold a previous run and could not be "
                    "cleared, so a crash here would have been read as that run's success: "
                    + "; ".join(stale)]}
    ...
    # Two spellings of the same two directories, and the difference is the whole point.
    # The rooted ones are evidence: a reader on another machine can find these under
    # their own checkout, and `_handle_base` resolves them back on `--reanalyse`. The
    # ones under RUNTIME_ONLY_KEY are the live channel: absolute, correct for this
    # process, and dropped by `public_payload` before anything is written. Neither can
    # quietly become the other, which is what let a rooted path be opened as a real one
    # and a wrong arm go uncompared.
    rec["outputs_dir"] = public_path(dump)
    rec["scratch_dir"] = public_path(scratch)
    rec[RUNTIME_ONLY_KEY] = {"outputs_dir": str(dump.resolve()),
                             "scratch_dir": str(scratch.resolve())}
    return rec


#: Top-level `cross_arm_equivalence` verdicts. The reason this field exists at all is
#: that its absence was indistinguishable from a pass: the old function returned
#: ``{"arms": {}}`` both when every arm matched and when it could not read the reference
#: tensor, and `compare_workload` — asked only "which arms are disqualified?" — read the
#: second as the first.
EQ_COMPARED = "COMPARED"
EQ_REFUSED = "REFUSED"
...
def cross_arm_equivalence(records: "list[dict]", reference_arm: str = ARM_CPU_HOST) -> dict:
    ...
    FAILS CLOSED, IN BOTH DIRECTIONS.  Every arm this function was asked about appears in
    ``arms`` with a verdict, whatever happened. An unreadable reference, an unresolvable
    handle, a manifest that names no file, a comparison that ended up looking at zero
    tensors — each of those is an explicit ``INSTRUMENT_ERROR``/``UNMEASURED`` entry for
    every arm it affects, and `compare_workload` refuses a speed claim for any arm whose
    entry is not ``MATCH``.

    The alternative, which this replaces, was to return early with an empty ``arms`` map
    and a ``detail`` string. Nothing downstream read ``detail``, so an equivalence check
    that never ran and an equivalence check that passed produced the same object — and a
    Vulkan arm whose logits were a thousand times the reference's was published as
    ``VULKAN_FASTER``. ``verdict`` and the per-arm entries exist so that "not compared"
    can no longer be spelled the same way as "compared and fine".
    """
    import numpy as np

    by_arm = {r["arm"]: r for r in records}
    subjects = [r for r in records if r.get("arm") != reference_arm]
    result: dict = {"reference_arm": reference_arm, "verdict": EQ_COMPARED,
                    "subject_arms": sorted({r["arm"] for r in subjects}), "arms": {}}

    def refuse_all(detail: str, verdict: str = INSTRUMENT_ERROR) -> dict:
        """Every subject arm carries the refusal, so no caller can miss it."""
        result["verdict"] = EQ_REFUSED
        result["detail"] = detail
        for rec in subjects:
            result["arms"][rec["arm"]] = {"verdict": verdict, "detail": detail,
                                          "outputs": []}
        return result
    ...
    if ref is None or not ref.get("outputs_manifest"):
        return refuse_all(
            f"reference arm {reference_arm} produced no outputs; every other arm is "
            f"UNMEASURED against it, and an unmeasured arm is disqualified from a speed "
            f"claim rather than assumed correct", UNMEASURED)

    ref_entry = ref["outputs_manifest"][0]
    ref_path, why = resolve_arm_output(ref, ref_entry)
    if ref_path is None:
        return refuse_all(
            f"the reference arm's output could not be read: {why}. No arm can be compared "
            f"against a reference that is not there")
    try:
        ref_arr = np.load(ref_path)
    except Exception as exc:
        return refuse_all(f"the reference arm's output at its own handle could not be "
                          f"loaded: {exc!r}")
    ...
    for rec in subjects:
        ...
        if not manifest:
            result["arms"][arm] = {"verdict": UNMEASURED, "outputs": [],
                                   "detail": "arm produced no comparable output"}
            continue
        path, why = resolve_arm_output(rec, manifest[0])
        if path is None:
            result["arms"][arm] = {"verdict": UNMEASURED, "outputs": [], "detail": why}
            continue
        try:
            arr = np.load(path)
        except Exception as exc:
            result["arms"][arm] = {"verdict": INSTRUMENT_ERROR, "outputs": [],
                                   "detail": f"this arm's output could not be loaded: {exc!r}"}
            continue
        ...
        res = compare_outputs([arr], [ref_arr], [ref_entry["name"]], regime=regime)
        if not res.get("outputs"):
            # A comparison that looked at nothing is not a comparison. Reached when the
            # two sides have different arity or one is absent: `compare_outputs` says so
            # in `detail`, and the verdict it returns for that case must not be allowed to
            # read as a pass here.
            res = {"verdict": INSTRUMENT_ERROR, "outputs": [],
                   "detail": (f"the comparison against the reference examined zero tensors "
                              f"({res.get('detail') or res.get('verdict')}), so this arm is "
                              f"unchecked rather than equivalent")}
        res["compute_regime"] = regime
        res["compute_regime_inferred"] = inferred
        result["arms"][arm] = res

    if any(v.get("verdict") != "MATCH" for v in result["arms"].values()):
        result["verdict"] = EQ_REFUSED
    result["compared_arms"] = sorted(
        arm for arm, v in result["arms"].items() if v.get("outputs"))
    return result


def repeat_key(rec: dict) -> str:
    """The repeat a record belongs to, as the string a JSON document can key on.

    A record with no ``repeat`` is repeat ``0``: `run_suite` stamps every record it
    dispatches, and a record predating that field came from a single-repeat run.
    """
    try:
        return str(int(rec.get("repeat") or 0))
    except (TypeError, ValueError):
        return str(rec.get("repeat"))


def workload_equivalence(records: "list[dict]",
                         reference_arm: str = ARM_CPU_HOST) -> dict:
    """Output equivalence for EVERY repeat of one workload, folded to a closed verdict.

    WHY PER REPEAT, AND WHY THE REPEAT SURVIVES THE FOLD.  This used to compare repeat 0
    and nothing else, while `compare_workload` pooled the ``steady_ms`` of every repeat
    into the ratio it published. So the samples that decided the winner and the tensors
    that were checked were different sets: a Vulkan arm that computed the right answer on
    repeat 0 and a thousandfold-wrong one on repeat 1 contributed both repeats' timings to
    a ``VULKAN_FASTER`` verdict, and the second repeat's outputs were never opened. That
    is Gate 4 failing open by a different door from the one it was closed at — the check
    was running, over a smaller world than the claim.

    So every repeat is compared, and the result keeps the repeat identity rather than
    collapsing it: ``repeats["0"]``, ``repeats["1"]`` … each hold a full
    :func:`cross_arm_equivalence` result, and ``arms[arm]["repeats"]`` is that arm's
    verdict per repeat. Nothing is keyed by arm name alone — a fold that writes
    ``arms[arm] = verdict`` inside a loop over repeats keeps whichever repeat happened to
    be last, which is how a divergent repeat becomes invisible without anybody deciding
    it should be.

    The fold is closed in the direction that refuses. An arm is ``MATCH`` for the workload
    only if EVERY repeat it appears in says ``MATCH``; one divergent, unresolvable or
    missing repeat makes the arm's workload verdict that refusal, and the workload verdict
    ``REFUSED``. An empty per-repeat verdict is a refusal, not a pass, for the same reason
    an empty ``arms`` map is.
    """
    by_repeat: "dict[str, list[dict]]" = {}
    for rec in records or []:
        by_repeat.setdefault(repeat_key(rec), []).append(rec)

    result: dict = {"reference_arm": reference_arm, "verdict": EQ_COMPARED,
                    "repeats": {}, "arms": {}, "subject_arms": [], "compared_arms": []}
    if not by_repeat:
        result["verdict"] = EQ_REFUSED
        result["detail"] = ("no records were supplied, so nothing was compared; an "
                            "equivalence result over zero repeats is a refusal rather "
                            "than a workload in which every arm agreed")
        return result

    def _sort_key(k: str):
        return (0, int(k)) if k.lstrip("-").isdigit() else (1, k)

    order = sorted(by_repeat, key=_sort_key)
    subjects: "set[str]" = set()
    for rep in order:
        eq = cross_arm_equivalence(by_repeat[rep], reference_arm)
        result["repeats"][rep] = eq
        subjects.update(eq.get("subject_arms") or [])
        if eq.get("verdict") != EQ_COMPARED:
            result["verdict"] = EQ_REFUSED

    result["subject_arms"] = sorted(subjects)
    result["repeat_ids"] = list(order)
    for arm in result["subject_arms"]:
        per_repeat: "dict[str, str]" = {}
        details: "list[str]" = []
        for rep in order:
            entry = ((result["repeats"][rep].get("arms") or {}).get(arm) or {})
            verdict = entry.get("verdict")
            present = any(r.get("arm") == arm for r in by_repeat[rep])
            if not present and not verdict:
                # The arm was not dispatched in this repeat, so it contributes no samples
                # to it either. `compare_workload` checks contribution against this map
                # per (arm, repeat), so an absence here cannot excuse a sample.
                continue
            if not verdict:
                verdict = UNMEASURED
                details.append(
                    f"repeat {rep}: this arm has a record and no equivalence verdict; an "
                    f"empty verdict is a refusal, not a pass")
            elif verdict != "MATCH":
                details.append(f"repeat {rep}: {verdict} — {entry.get('detail') or 'no detail'}")
            per_repeat[rep] = verdict
        if not per_repeat:
            folded = UNMEASURED
            details.append("this arm carries no per-repeat equivalence verdict at all")
        else:
            divergent = [v for v in per_repeat.values() if v != "MATCH"]
            folded = "MATCH" if not divergent else divergent[0]
        result["arms"][arm] = {
            "verdict": folded,
            "repeats": per_repeat,
            "detail": "; ".join(details),
    ...
    result["compared_arms"] = sorted(
        arm for arm, v in result["arms"].items() if v["verdict"] == "MATCH")
    refusing = [f"{arm}: {v['detail']}" for arm, v in result["arms"].items()
                if v["verdict"] != "MATCH" and v["detail"]]
    top = [result["repeats"][rep].get("detail") for rep in order
           if result["repeats"][rep].get("detail")]
    if result["verdict"] == EQ_REFUSED:
        result["detail"] = "; ".join(refusing + top) or (
            "at least one repeat refused; see `repeats` for which")
    return result
...
def run_suite(workloads: "list[Workload]", arms: "list[str]", *, iters: int, warmup: int,
    ...
    for w in workloads:
        ...
        eq = workload_equivalence(per_workload)
...
def compare_workload(workload: Workload, records: "list[dict]",
    ...
    AN ARM THAT WAS NEVER CHECKED IS EXCLUDED TOO, AND THAT IS THE POINT.  This used to
    ask one question — "which arms did `cross_arm_equivalence` disqualify?" — and an
    empty answer meant "none". But an empty answer was also what came back when the
    equivalence check could not run at all: no reference tensor, an unresolvable handle,
    or no equivalence argument passed. Silence read as consent, and the suite published
    ``VULKAN_FASTER`` for an arm returning logits a thousand times too large. So the
    question asked here is now the closed one: *every* contending arm must carry a
    ``MATCH``, and an arm with no verdict at all is refused by name.

    THE UNIT OF THAT QUESTION IS (ARM, REPEAT), NOT ARM.  The pool below draws
    ``steady_ms`` from every repeat, and the equivalence result used to describe repeat 0
    alone. An arm therefore only had to be right ONCE to have all of its samples counted.
    So the check is made against the same index the samples come from: every repeat that
    contributes a sample must carry a ``MATCH`` for that arm in
    ``equivalence["repeats"][<repeat>]``, and a repeat that contributes samples and has no
    entry there is refused by name.

    AND A FLAT MAP SPEAKS FOR AT MOST ONE REPEAT.  A repeat-blind ``{arm: verdict}`` result
    — what a single-repeat run and every stored suite predating this contract produce — was
    honoured for the arms it named, whatever the number of repeats those arms contributed.
    That is the original defect with one layer of indirection: the protocol in
    ``docs/PERF.md`` runs ``--repeats 3``, so an arm-only verdict was silently standing in
    for three separate comparisons of three separate output sets, and a divergent second
    repeat was invisible again. The compatibility path is therefore CONSTRAINED, not
    removed: it is honoured only where it cannot be wrong — an arm contributing exactly one
    repeat, which is the case it was written for — and an arm contributing more than one
    repeat under a flat map is refused by name, arm and repeat.
    """
    equivalence = equivalence or {}
    eq_arms = equivalence.get("arms") or {}
    eq_repeats = equivalence.get("repeats") or {}
    # `!= "MATCH"`, not `not in ("MATCH", None)`: a verdict-less entry is an arm nothing
    # concluded anything about, which is exactly the state that must not pass.
    disqualified = {arm for arm, res in eq_arms.items()
                    if res.get("verdict") != "MATCH"}

    #: ``{arm: {repeat: n_samples}}`` — the index the ratio is actually built from.
    contributing: "dict[str, dict[str, int]]" = {}
    for r in records:
        if r.get("verdict") != ADMISSIBLE:
            continue
        n = len(r.get("steady_ms") or [])
        if not n:
            continue
        contributing.setdefault(r.get("arm"), {}).setdefault(repeat_key(r), 0)
        contributing[r.get("arm")][repeat_key(r)] += n

    def _repeat_verdict(arm: str, rep: str) -> "str | None":
        """This arm's verdict for this repeat, or ``None`` when nothing spoke for it."""
        per_repeat = (eq_arms.get(arm) or {}).get("repeats") or {}
        if rep in per_repeat:
            return per_repeat[rep] or None
        rep_eq = eq_repeats.get(rep) or {}
        entry = (rep_eq.get("arms") or {}).get(arm) or {}
        if entry.get("verdict"):
            return entry["verdict"]
        if eq_repeats or per_repeat:
            # The equivalence result knows about repeats and has nothing for this one.
            return None
        if len(contributing.get(arm, {})) != 1:
            # A flat, repeat-blind result over an arm that contributed more than one
            # repeat. It was produced by comparing ONE output set; honouring it here would
            # let that one comparison acquit every other repeat's tensors, which is the
            # defect this whole check exists for wearing a compatibility label.
            return None
        # The one case a flat result cannot be wrong about: a single contributing repeat,
        # which is what a single-repeat run and every stored suite predating this contract
        # produce. Constrained rather than removed, and asserted in both polarities by
        # `bench/test_cuda_competition.py`.
        return (eq_arms.get(arm) or {}).get("verdict") or None

    unrepresented: "list[str]" = []
    unrepresented_arms: "set[str]" = set()
    divergent_repeats: "list[str]" = []
    for arm in (ARM_VULKAN, ARM_CUDA):
        for rep in sorted(contributing.get(arm, {})):
            verdict = _repeat_verdict(arm, rep)
            if verdict is None:
                unrepresented.append(f"{arm}@repeat{rep}")
                unrepresented_arms.add(arm)
            elif verdict != "MATCH":
                divergent_repeats.append(f"{arm}@repeat{rep}={verdict}")
    ...
    contenders = {name for name, s in ((ARM_VULKAN, vk), (ARM_CUDA, cu)) if s}
    unchecked = sorted((contenders - set(eq_arms)) | (contenders & unrepresented_arms))
    ...
    entry: dict = {
        ...
        "equivalence_verdict": equivalence.get("verdict"),
        "equivalence_disqualified_arms": sorted(disqualified),
        "equivalence_unchecked_arms": unchecked,
        "equivalence_unchecked_repeats": sorted(unrepresented),
        "equivalence_divergent_repeats": sorted(divergent_repeats),
        "contributing_repeats": {arm: dict(sorted(reps.items()))
                                 for arm, reps in sorted(contributing.items())},
        "verdict": UNMEASURED,
    }
    if disqualified & {ARM_VULKAN, ARM_CUDA}:
        entry["detail"] = (
            f"arm(s) {sorted(disqualified & {ARM_VULKAN, ARM_CUDA})} did not match the CPU "
            f"reference within the derived ULP budget"
            + (f" (repeat-level: {sorted(divergent_repeats)})" if divergent_repeats else "")
            + "; a speed ratio between arms computing different answers is not a "
              "performance result")
        entry["verdict"] = NOT_EQUIVALENT
        return entry
    if unchecked:
        entry["detail"] = (
            f"arm(s) {unchecked} produced timings but carry no output-equivalence verdict "
            f"for every repeat those timings came from "
            f"(unrepresented: {sorted(unrepresented) or 'the arm itself'}; "
            f"{equivalence.get('detail') or 'no equivalence result was supplied'}); an "
            f"unchecked repeat is not an equivalent repeat, and no ratio is quotable")
        entry["verdict"] = INSTRUMENT_ERROR
        return entry
    if not vk or not cu:
...
def render(suite: dict) -> str:
    ...
    lines.append("")
    lines.append("| workload | repeat | arm | verdict | regime | max peak-ULP | budget | raw ULP | "
                 "max abs diff | top-k | resolvable rows |")
    lines.append("|---|---|---|---|---|---|---|---|---|---|---|")
    for wl, eq in (suite.get("equivalence") or {}).items():
        # PER REPEAT, AND THE REPEAT IS IN THE ROW.  A folded "the vulkan arm MATCHed"
        # is the sentence that let repeat 1 go unread while repeat 1's samples decided
        # the ratio; the reader has to be able to see which repeats were compared.
        per_repeat = eq.get("repeats") or {"-": eq}
        for rep, rep_eq in per_repeat.items():
            for arm, res in (rep_eq.get("arms") or {}).items():
                ...
                lines.append(
                    f"| {wl} | {rep} | {arm} | {res.get('verdict')} | "
                    f"{res.get('compute_regime','-')} | "
        ...
        folded = eq.get("arms") or {}
        if eq.get("repeats") and folded:
            lines.append("")
            lines.append(f"Folded over every repeat of `{wl}` — an arm is quotable only "
                         f"if EVERY repeat that contributed a timing sample says MATCH:")
            for arm, res in folded.items():
                per = res.get("repeats") or {}
                spelled = ", ".join(f"repeat {k}={v}" for k, v in per.items()) or "no repeat"
                lines.append(f"- `{arm}` → **{res.get('verdict')}** ({spelled})"
                             + (f" — {res['detail']}" if res.get("detail") else ""))
            lines.append("")
    refused = [(wl, eq.get("detail"))
               for wl, eq in (suite.get("equivalence") or {}).items()
               if eq.get("verdict") == EQ_REFUSED and eq.get("detail")]
    if refused:
        # A refusal that only exists as a verdict in a table cell reads as one arm's bad
        # luck. It is a statement about the whole workload, so it gets a sentence.
        lines.append("")
        lines.append("**Equivalence refusals** — no arm in these workloads is quotable "
                     "for speed, because the check did not run rather than passed:")
        for wl, detail in refused:
            lines.append(f"- `{wl}` — {detail}")
    lines.append("")
