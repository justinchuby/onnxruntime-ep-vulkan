...
def _worker_main(argv: "list[str]") -> int:
    ...
    if workload is None:
        Path(a.out).write_text(json.dumps({
            ...
            "instrument_errors": [f"unknown workload {a.workload!r}"]}), "utf-8")
        return 3
    ...
    Path(a.out).write_text(json.dumps(payload, indent=2, default=str), "utf-8")
...
def main(argv: "list[str] | None" = None) -> int:
    ...
    if a.reanalyse:
        ...
        out.write_text(json.dumps(suite, indent=2, default=str), "utf-8")
        report = render(suite)
        out.with_suffix(".md").write_text(report, "utf-8")
        print(report)
        print()
        print(f"record: {out}")
        return 0
