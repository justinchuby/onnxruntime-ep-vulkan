...
def compare_workload(workload: Workload, records: "list[dict]",
    ...
    entry there is refused by name. A flat ``{arm: verdict}`` map — which is what a
    single-repeat run and every stored suite predating this produce — is honoured for the
    arms it does name, and any contributing repeat it cannot speak for is unchecked.
    """
    ...
    def _repeat_verdict(arm: str, rep: str) -> "str | None":
        ...
        if eq_repeats or per_repeat:
            ...
            return None
        # A flat, repeat-blind result (a single-repeat run, or a stored suite from before
        # this contract). It speaks for the arm as a whole or not at all.
