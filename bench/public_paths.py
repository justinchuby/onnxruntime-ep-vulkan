...
def contained_child(base, rel) -> "Path | None":
    ...
    file this one opens. ``rel`` is a *relative* name by contract, so everything that
    is not one is refused rather than repaired: an absolute path, a drive-relative
    ``C:out0.npy``, a rooted ``<repo>/…`` token, an empty component, ``.`` or ``..``.

    The syntactic checks are not the guarantee — ``resolve()`` followed by
    :func:`_relative_to` is. A symlink inside *base* pointing at ``C:\\Windows`` passes
    every string test and fails this one, which is why containment is decided on the
    resolved parts and not on the text.
    ...
    if not text or text.startswith("/") or _ROOTED.match(text):
        return None
    head = text.split("/")[0]
    if ":" in head:
        return None
    ...
    except OSError:  # pragma: no cover - an unresolvable handle is refused, not guessed
        return None
