...
def unusable_base(base) -> "str | None":
    """Why *base* cannot root a handle, or ``None`` when it can.

    ONE PREDICATE, BOTH HALVES OF THE CONTRACT.  :func:`contained_child` (the reader) and
    `cuda_competition.relative_handle` (the writer) are the two ends of the same rule, and
    they disagreed about the empty base: the reader refused it and the writer resolved it
    to the process's current directory and happily produced a handle. That asymmetry is a
    handle written against a directory nobody named, which the reader will then refuse —
    the record carries a name no second process can resolve, and "the file was never
    written" and "the base was never named" arrive spelled the same way.

    Totality is the reason this is a predicate and not an exception: both callers return
    their refusal so the caller can record the ``why`` beside the absent handle.
    """
    if base is None:
        return ("no directory was given to root this handle at; a handle is a name INSIDE "
                "a directory and there is no directory here")
    if isinstance(base, (bool, int, float)):
        return (f"the base {base!r} is not a path; coercing it would invent a directory "
                f"name out of a number")
    try:
        text = str(base)
    except Exception:  # pragma: no cover - an object whose __str__ raises
        return "the base could not be read as a path at all"
    if not text.strip():
        return ("the base is empty, which resolves to whatever directory this process "
                "happens to be running in. A handle rooted at the current directory is "
                "not a handle rooted at the directory the record names")
    return None
...
def contained_child(base, rel) -> "Path | None":
    ...
    Refused: a non-string; a base :func:`unusable_base` names (absent, empty, not a path);
    an absolute, rooted (``<repo>/…``),
    ...
    * **No repair.** Every check above is a case where the previous version handed back
    ...
    if not isinstance(rel, str):
        ...
        return None
    if unusable_base(base) is not None:
        return None
    text = rel
