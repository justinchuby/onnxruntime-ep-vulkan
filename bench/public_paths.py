...
def scan_structural(text: str) -> "list[tuple[str, str]]":
    ...
    whole repository it makes the answer a function of the runner: ``justinchu`` finds 292
    files, a CI account called ``runner`` finds none of those and some others, and the
    checked-in ratchet those numbers are compared against is then either stale, or
