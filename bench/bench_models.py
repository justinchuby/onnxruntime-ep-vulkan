...
def _resolve_url(spec: ModelSpec, *, allow_download: bool) -> ResolvedModel:
    ...
    if missing and not allow_download:
        ...
        return ResolvedModel(
            ...
            detail=(f"{names} absent from {root}. Pass --allow-download to fetch from the "
                    f"recorded public URL(s), or set {CACHE_ENV} to a directory that has "
                    f"them. This harness never downloads implicitly."),
            license_id=spec.license_id, license_url=spec.license_url,
