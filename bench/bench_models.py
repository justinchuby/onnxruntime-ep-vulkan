...
Phi-3.5 graph proto is 26 MB (``model.bytes`` 26180848) and the weights it
references are 2.29 GB (``model.weights_bytes`` 2291238912) in a sibling
``.onnx.data``; both figures are read off the committed witness
``bench/results/real_model_gqa_local_size.json``, whose
``model.external_data.files[0]`` names the ``.onnx.data`` blob and its own
digest.  Each figure is quoted beside the field it was read from, in its own
clause, because a figure whose provenance sits in the next sentence is a figure
a reader has to guess at -- and, until `bench/test_cuda_competition.py`'s
numeric-witness guard stopped pooling citations over the paragraph, a figure the
guard would bless from a neighbour's field.  The rounding matters too: a coarser
one printed here previously disagreed with the witness in the digit it printed,
which is how a provenance note stops being provenance.  A digest of the proto
alone would call two different weight sets "the same model", which is exactly
the substitution this module exists to make impossible.
"""
